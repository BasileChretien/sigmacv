...
for pdir in parts:
    ...
    con.execute(f"""
      COPY (
        ...
        FROM read_json('{gz}', format='newline_delimited',
                       maximum_object_size=50000000, ignore_errors=true)
      ) TO '{outpq}' (FORMAT parquet, COMPRESSION zstd);
    """)
    done += 1
    print(f"[{done}/{len(parts)}] {nm} {round(time.time()-t,1)}s")

n = con.execute(f"SELECT count(*), count(orcid), count(country) FROM '{PARTS}/*.parquet'").fetchone()
