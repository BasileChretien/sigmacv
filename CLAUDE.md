...
- [ ] **Preview analytics claim** — the privacy notice says analytics never record which ORCID iD was previewed. (The same scrub cuts the path from outbound-link URLs to `shareyourpaper.org` and `doi.org`: outbound-link tracking is ON in the site's Plausible config, verified in the live `pa-*.js` on 2026-09-15, and would otherwise send a clicked paper's DOI.) That rests on (a) the client-side scrub in `src/lib/analytics/plausibleInit.ts` (verified 2026-09-09 against the live `pa-*.js` tracker: `transformRequest` is read from the init options) and (b) the idempotent ClickHouse purge `scripts/deploy.sh` runs on every deploy. After any Plausible upgrade, open a preview and confirm the outgoing `POST …/api/event` body carries `/preview/_`, not an iD. `PREVIEW_SUPPRESSED_ORCID_HMACS` (the Art. 21 objection list) is keyed by the dedicated `PREVIEW_SUPPRESSION_KEY` (required whenever the list is set; `env.ts` fails loudly otherwise) — `AUTH_SECRET` rotation does not touch it; rotating the key means recomputing every entry.
