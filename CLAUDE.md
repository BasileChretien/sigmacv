...
- [ ] **Narrative AI drafting (optional, OFF by default)** — set `AI_NARRATIVE_ENABLED=true` **and** `AI_API_KEY` (default provider Mistral AI, France — EU data residency) to offer the opt-in first-draft on the narrative modules (`src/lib/ai/*`, endpoint `/api/cv/narrative-draft`). With either unset the endpoint 404s and makes **no** network call. Pluggable to any OpenAI-compatible EU endpoint via `AI_BASE_URL`/`AI_MODEL` (Scaleway, OVHcloud). Keep it consented + disclosed (the privacy notice names the processor); the request sends only a public-only summary of the user's own outputs (no contacts/identifiers) and nothing is stored.
