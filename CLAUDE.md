...
- [ ] **FORRT replications (optional, DORMANT)** — the `ForrtReplication` table is empty by design: no dataset ships in the repo. To enable replication evidence: (1) download the FORRT Replication Database (FReD) CSV export linked at <https://forrt.org/replication-hub/> (CC-BY — **cite FORRT/FReD** wherever the data is shown or reused, per the licence); (2) run `npm run forrt:import <path.csv[.gz]>` against the **production** `DATABASE_URL` (same checkout caveat as `oep:import`; the importer's column matching is defensive/tolerant — see the `TODO(verify-live)` notes in `scripts/forrt-import.ts` and re-check against a real export header before the first run); (3) **re-run on each FReD refresh** (no fixed cadence is published — check the OSF project for update frequency). Empty table = the feature is a no-op (`display.showReplications` has nothing to show). Unlike WHO ICTRP, redistribution is allowed by FReD's licence, so a committed seed _could_ ship in future — none does yet.
