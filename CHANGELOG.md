...
- **Entries you had cited with the old button now become contribution cards too.** Before contributions were cards, "Cite one of my entries" dropped a marker at the top of the section. A paragraph made only of such markers is now turned into one card per entry, in order, when the CV opens, and an entry that already has a card is not added twice.
- **Contributions are cards you edit, move and delete, and they print as a proper list.** Each entry you add under a contributions section (the FRQ's second section, the Tri-agency's "Most significant contributions", R4RI's knowledge module) is now a card, not lines of text: its title and reference come live from your record, and you write the period, the audience (A, B, C), your role, the impact and where the work is cited, each line with its own link. Cards move up and down and are deleted with one click; a contribution that is not one of your entries takes a title of its own. A picked work starts with its year and the clinical guidelines that cite it, which you can edit or remove, and it is no longer offered in the picker. On the CV, in PDF, DOCX, LaTeX and Markdown alike, each contribution is a numbered block: the title, the period and audience spelled out, the role and impact under their labels, the places it is cited as links, and the reference set apart, your name in bold. Existing text is cleaned up when the CV opens: an entry you had picked becomes a card with what you wrote for it, a suggested entry you had written a role or impact for becomes a card too, and the ten suggestions nobody touched disappear.
...
- **Publisher policies now come from Jisc Open Policy Finder.** For each closed journal article in the Open access tab, the owner's sync asks Open Policy Finder, by the journal's ISSN, what the publisher allows: which version, where, after what embargo, under what licence and conditions. Its records are maintained (most updated in 2025–2026, against 2019–2022 for OA.Works) and cover journals OA.Works had nothing on, so more papers get a publisher's permission instead of relying on the law alone. Where a journal offers several routes, each paper is shown the one it can take today, for the place its deposit goes (a Zenodo deposit is never shown a route that allows only an institutional repository). A journal that has since become fully open access is not used for your older closed papers, whose terms were different. OA.Works remains the fallback for a journal Open Policy Finder does not know. Every line says which source it comes from, and an Open Policy Finder record links the journal's record there with the date retrieved, as agreed with Jisc: shown only to you in your editor, never on your public page or in a download (the JSON and RO-Crate exports and the account export leave it out). HAL is no longer counted as a "subject repository" when a policy names only that kind of place.
- **Each paper in the Open access tab now says which file to upload.** Under the deposit action, one line spells out the document in plain words: your own manuscript as accepted after peer review, without the journal's copy-editing and layout, and not the publisher's PDF; your manuscript as first submitted; or the publisher's PDF itself, when the licence or the policy allows it. The line follows the action's own choice of version, and is left out when the action itself is only "if your publishing agreement allows it". The sentence under the file line that says why the deposit is allowed is shorter: it gives the ground alone (the publisher's record, the law, the licence), since the action and the file line already name the version.
...
- **Co-author names print the way the paper printed them.** Three kinds of glitch in the sources reached the citations: a capital after an accented letter ("ChréTien", what a title-caser leaves when it takes "é" for a word break), a character lost in an upstream decode ("Kenji U�da" for Kenji Uda), and an OpenAlex author profile named in another script than the paper ("Осаму Сузукі" for Osamu Suzuki, on an English paper). Names are now repaired before they reach any citation, in every format: the casing is fixed, a name garbled by a wrong character encoding ("ChrÃ©tien") is decoded back, and when OpenAlex's profile name is garbled or in another script, the name printed on the paper is used instead. Each repair leaves a sound name alone: all-caps names, real internal capitals ("McDonald", "Ó hUiginn") and Czech or German names whose letters merely look like an encoding fault ("LÍŠKA", "Strauß–Meyer") are kept as written. An author list the publisher deposited twice (the RECOVER study names five of its authors twice, in Crossref and in OpenAlex alike) is printed once. The bar for that is high, because dropping a real co-author is worse than naming one twice: at least three names repeated in the same order, never two people with different ORCID iDs, never on a byline of more than 500 authors. Stored CVs get the repaired names as they open, entries added by DOI or found through ORCID included; a doubled list or a name in the wrong script is corrected at the work's next sync. A frozen version of a CV keeps its citations exactly as they were frozen.
- **A HAL notice no longer asks for a version the publisher's policy does not allow there.** When HAL already holds a notice of a paper without a file, the Open access tab sends you to that notice. If the publisher's policy was the only ground for the deposit, and it names only places HAL is not (a preprint server, say), the row still said "Add the published version to the HAL notice …". It now says "Add your file to the HAL notice … only if your publishing agreement allows it", as every other place the policy does not cover does, and names no file to upload. When the law or the paper's own licence is the ground, the row names the version as before. In Spanish, every HAL-notice action now addresses you as "tú", like the rest of the tab.
...
- **SigmaCV now checks whether a paper is already in a repository — HAL, Europe PMC, an OpenAIRE-harvested repository — before asking you to deposit it.** OpenAlex, whose word "no open copy found" was, misses deposits routinely: on one CV, 17 of 51 closed articles had a HAL record it did not know — 3 with a file (one since 2021), 14 notices without one — and Unpaywall saw none of them either. The owner's sync now asks the repositories themselves, for every paper of the Open access tab in one sync: HAL for all of them first, then Europe PMC and OpenAIRE for what HAL did not settle (budgeted, fail-soft, OpenAIRE with the configured access token). A copy with a file anywhere means the paper is open in a repository: it leaves both lists. A HAL notice without a file changes the HAL action to "Add the accepted manuscript to the HAL notice hal-…", linking the notice, with a note — a fresh deposit would duplicate it. A paper the check has not reached yet sits in a closed fold, "Papers not checked yet", by title only, rather than being called a paper with no open copy. Owner-only, stripped from every public surface like the rest of the worklist's inputs.
- **"Draft with AI" can read the call you are answering.** A new box under the provider fields takes the text of the funding call, job posting or programme guidance you are writing for. The draft then speaks to what that call says it assesses, in the call's own vocabulary, instead of a generic account of your work. The text goes to your own provider with the rest of the prompt, as reference material the model is told not to take instructions from, and is remembered in your browser only, so the same call serves every module of the document. Nothing changes when the box is empty.
- **An asterisk after the people you supervised, in your references.** The FRQ asks for it in its CV descriptif ("Nom, Prénom\*"), and the CV-FRQ layouts now do it: every co-author of one of your works whose name matches one of your supervision records gets an asterisk after the name, in the form the citation style prints ("Kaur, P.\*", "P. Kaur\*", "Kaur P\*"), in PDF, HTML, DOCX, LaTeX and Markdown alike. It is a switch in the Design panel, off by default; the FRQ layouts turn it on and hand your own setting back when you leave them. It is the one place SigmaCV matches a person by name rather than by identifier, because a supervisee has no identifier in the data and the names are the ones you typed yourself; the switch says so, and warns that a co-author with the same name would be marked too. It stays off while supervisee names are hidden, since the asterisk would reveal the relationship that toggle keeps off the page.
...
- **A public page under a funder layout describes itself the way it looks.** The CV-FRQ and Tri-agency layouts leave the headline and summary off the document (#496), but the living page's meta description, its social card and its JSON-LD `jobTitle`, and the ribbon of the marquee style, still read them from the owner's profile, so the page said one thing and its metadata another. Every surface that prints the headline or summary now reads one helper, `headerSummary`, which returns nothing when the layout hides them; the description falls back to the name and the latest affiliation, the card shows no headline, the JSON-LD carries no job title. The owner's text is untouched.
...
- **The CV-FRQ sections could be read on the CV but not found in the editor, and reappeared under other saved views.** A layout's narrative sections arrived collapsed in the Content column, and a collapsed prose card is only its title and a small chevron, so the pre-filled text (a starter draft since #494) showed on the page while the column seemed to offer nothing to edit; before the pre-fill an empty section had auto-expanded. Every visible prose section now opens the first time it appears. And a saved view, including the automatic "Before CV model" one, kept showing prose sections created after it was saved, because a section it had never seen kept its current visibility: such a prose section is now hidden by the view (a data section a later sync added still keeps its own visibility).

### Added

- **A second, folded list on the Open access tab: papers open at the publisher, to put in a repository too.** A paper that is gold, hybrid, diamond or bronze at the publisher is not in a repository yet, and a French researcher's HAL still expects it. Under "Papers open at the publisher — put a copy in a repository too", closed on every visit, each such paper gets the same three-line row as the first list, with its ground: the work's own licence ("Allowed by the work's licence (cc-by): the published version." — the form takes that licence, and the file is the publisher's PDF), else the publisher's record, else a statutory right whose delay has run. Green papers are left out: their copy is in a repository already. A bronze paper with no licence and no right is not listed. The chips on the publication rows stay with the first list.
- **The CV-FRQ document looks like the FRQ's, and says how long it is.** The FRQ template has no personal statement outside its three sections and the FRQ never asks for a figure, yet the layout still printed the header summary and headline above the sections and would have carried the research-summary block if the owner had metrics on. The CV-FRQ and Tri-agency layouts now leave the headline and summary off the page (`display.hideHeaderSummary`, cleared by any other layout and by the reset; the owner's text is untouched), hide the research-summary block, and set letter paper, the size FRQnet's presentation standards fix for every attachment; the owner's own paper size and block placement are kept aside and come back with the next layout that does not set them, or with the reset. Those standards also fix the typography (2 cm margins, Times 12 point, single spacing), so a page holds about 3,400 characters: each prose section's editor now shows "≈ n pages" beside its character counter, and the CV-health panel adds "≈ 5.4 of 6 pages" for the narrative sections together, turning to a warning past the layout's limit (6 in French, 5 in English, carried as `display.pageLimit`). The estimate describes the funder's Word template the text is pasted into, not SigmaCV's preview.
- **Each closed paper says where to deposit it, on its own row.** In the editor's Content tab, a journal article with no open copy found now carries a small chip — for example "Deposit in HAL", or "Deposit in HAL if allowed" when the publisher's record allows no deposit outright — that opens the Open access tab at that work's row, where the reason, the DOI to copy and the policy notes live. The chip reads the same routes and the same record as the worklist and follows the same paper-or-current-affiliation choice, so the two never disagree; it exists for the owner only, and the no-login preview shows none.
- **A narrative layout no longer opens on a blank page.** Applying the CV-FRQ, the Tri-agency CV or the R4RI showed three or four empty prose sections and hid every list, and a prose section with nothing in it is not rendered, so the preview went blank and a reader of the editor asked where the sections had gone. Each empty prose section the layout shows is now pre-filled with a starter draft written from the owner's own entries, in the CV's language, with square-bracket prompts where the owner has to write: the background section lists education, positions, recognitions and funding; the contributions section carries up to ten numbered stubs, each with the period, an audience slot (A / B / C), a role slot, an impact slot and a plain-text reference, seeded from the most cited publications and the most recent datasets, software, patents and trials; the people section lists supervision and teaching records; the community and society modules list the relevant service records. A section that already has text is never touched, an empty one also offers the draft on demand from a button, and the freeze and snapshot paths never pre-fill, so a frozen document cannot carry a scaffold.
- **The CV-FRQ guide and the French example link to the official documents, in plainer words.** Both pages now list the FRQ's own pages, instructions and Word templates in each language, and the federal Tri-agency CV pages (CIHR, SSHRC, NSERC), under an "Official documents" heading; guides gain a `links` block for this. The copy of both pages was rewritten by hand: shorter sentences, no dashes, no groups of three, one claim per sentence, in all ten locales of the guide.
- **A guide to the Québec "CV descriptif", and a French example of one.** A librarian at Université de Montréal, fresh from presenting the format to her faculty, asked how SigmaCV shows impact in a narrative CV and whether it could share an example. The new guide "The FRQ narrative CV (CV-FRQ): how to prepare it" (ten locales, French and English hand-written) sets out what the Fonds de recherche du Québec instructions actually require — three sections under the FRQ's headings, six pages in French or five in English, up to ten contributions each with its period, audience A/B/C, role and impact, names in bold and an asterisk after each supervisee — and answers the impact question the way the FRQ's own text does: a claim is proved by a chain the reader can follow (output, what it changed, where that change is recorded), never by a citation count. The example gallery gains its first non-English page, a fictional early-career clinical pharmacologist's CV-FRQ in French: ten contributions from a hepatotoxicity signal that reached a product monograph and a regulator's safety communication, through a ministry memorandum and an open dataset, to a radio column that honestly claims no measured effect — every person, institution, drug, journal and DOI invented, the page chrome now following the example's language. The country-formats guide links to the new one, and an example's URL is no longer rewritten under a locale prefix it does not have.
- **The Québec "CV descriptif", and the Canadian model says what it is.** The Fonds de recherche du Québec has required a narrative CV in its competitions since 2025-2026 — three sections under the FRQ's own headings, no word limit, six pages in French or five in English, written in the FRQ's Word template and attached as a PDF to the FRQnet form, which collects education, employment and languages itself. SigmaCV had no Québec layout at all, and its one Canadian model was called "Canadian Common CV" and sent applicants to the CCV portal — the portal the federal agencies are retiring competition by competition in favour of the Tri-agency CV. Two FRQ models join the catalog (French and English, because the page limits differ), each exactly the three FRQ sections — "Parcours et compétences", up to ten "Contributions et expériences les plus importantes", "Activités de supervision et de mentorat" — with the headings verbatim from the FRQ instructions and nothing else shown, so the narrative-evidence panel does the work of linking each contribution to a real entry. The federal model is renamed "Tri-agency CV (CIHR · NSERC · SSHRC)" and its text now says the CCV is being replaced, with the same persisted id so existing snapshot and freeze requests keep resolving. The catalog count reads 61 everywhere the site quotes it (it had said 58 through two additions).
- **Entries keep the link ORCID records for them.** ORCID stores a URL with every activity — the team page behind an affiliated-researcher membership, a society's member listing, an award announcement, a department's staff page — and SigmaCV read every other field of those records and dropped that one, so a membership that pointed somewhere on ORCID pointed nowhere on the CV. Every entry built from an ORCID affiliation (employments, education and qualifications, distinctions, memberships and services, invited positions) now carries it as `meta.entryUrl`, and it is shown beside the entry: on screen and in the PDF as the link's host ("· neuropresage.fr"), styled like the institution's ROR link and sitting next to it on a Positions record's sub-line; in DOCX, LaTeX and Markdown as the whole URL in parentheses, since a printed CV cannot be clicked; and, for positions and education, as schema.org `url` on the published page's JSON-LD. It is the ENTRY's link, distinct from the institution's homepage that ROR resolves and that already sits behind the institution name — the two appear together, never nested. Only a plain http(s) URL is kept (ORCID accepts whatever the record holder types), and every renderer revalidates it. Every one of those rows gains a "Link (URL)" field in "Edit details" (ten locales) that overrides the recorded URL and reverts to it when cleared, like the role, department and institution fields beside it; the source value keeps refreshing underneath on every re-sync. The panel itself now reaches those sections at all: it was gated to Positions and Education, so a membership, an invited role or an award could only be hidden or reordered. Service & Memberships and Invited Talks are built exactly as Education is, so they get the same fields; an award’s line is a point-in-time form with no structured role or range behind it, so it is edited as one line of text plus its link — the structured fields would have rewritten "Fellowship, Royal Society (2019)" as "Royal Society".

- **Each closed paper in the worklist names one place to deposit it, and why.** The rights lines said what a researcher may do with a closed paper but not where to put it — and the choice between a funder's repository, HAL, an institutional repository and Zenodo is where most people stop. Under each closed journal article the owner worklist now gives one action: a verb, a link to the destination's deposit form and a one-clause reason ("Deposit the accepted manuscript in HAL — because of your affiliation on this paper (France)"), with the other places and ShareYourPaper behind a disclosure and a Copy DOI button for the form (ten locales). The order is the rule, never a score: a funder repository SigmaCV has confirmed for a funder the work names (NIH → PubMed Central through NIHMS and Wellcome → Europe PMC plus, each noting that a co-author may already have submitted it), then HAL, Zenodo or arXiv when OpenAlex lists some of the owner's works there, then the national repository of the affiliation country (France → HAL), then any other repository OpenAlex lists their works in — after the national one, because a co-author's institutional repository reaches that list as easily as the owner's own — then Zenodo, open to anyone. Those repositories come from two polite-pool OpenAlex calls in the owner's own sync (the repository-typed sources their works are located in; indexes, data platforms and preprint servers other than arXiv excluded; HAL portals merged into HAL; at most three kept as names and links, the counts that ordered them not stored), sharing an eight-second budget, refreshed weekly and never run for the anonymous preview. The routes follow the affiliation printed on each paper; an owner whose current affiliation is in another country can switch to it when that changes an action. The action's words never go beyond the publisher's record shown above it: the version the policy allows; with no record, the accepted manuscript "if the journal's policy allows it"; "only if your publishing agreement — or a right shown above — allows it" when the record allows no deposit or names only other kinds of repository. Under it come the licence and a still-running embargo to set in the form (Zenodo's form preselects CC BY), and where the publisher's DOI goes — for Zenodo, under related works; for HAL, into the box that fills the form in from it (HAL ignores a DOI passed in the link). Nothing here reaches the public page, an export or the harvest, and a click sends one cookieless analytics event carrying the route's kind only. Tables and their sources: `docs/DEPOSIT-ROUTES.md`.
- **Closed papers show what their publisher and the law allow for self-archiving.** The owner worklist listed each countable work with no open copy found beside a link to search the journal's policy, and stopped there: a researcher still had to find out whether the accepted manuscript could go in a repository, when, and under which licence, and had no way of knowing that the law of the country on their affiliation might grant a right the publishing contract does not. Each closed journal article now carries, in the editor only, the publisher's self-archiving policy as OA.Works records it — which versions, where, the embargo with its end date for that article, the licence of the deposited copy, the statement the publisher asks to include (quoted verbatim), the date OA.Works last updated the record and the date SigmaCV retrieved it — and, for the country printed on the owner's affiliation on the paper, the statutory rule that may also apply: France's article L533-4, Germany's § 38(4) UrhG, Austria's § 37a UrhG, the Dutch Taverne amendment and Belgium's article XI.196 under journal articles, Bulgaria's article 60(2)–(4) under works of scientific literature from 2021 on, Spain's repository-deposit requirement under works from 2022 on, and Japan's national policy under works from 2025 on — a rule is never shown under a work its year or type already rules out, and a country whose law gives the author nothing is simply not listed. Each rule is dated and linked to its text or to guidance, under one disclaimer: records age; the statutory conditions (public funding of the work or of the institution, the journal's periodicity, co-authors' agreement, the discipline) cannot be read from metadata; a signed agreement may allow more or less than the journal's general policy; and this is information, not legal advice. The lookup runs only in syncs of the owner's own document — the manual sync, the first visit to the editor and the scheduled re-sync of a published CV — never in the anonymous preview: one call per closed article, one at a time, at most 40 per sync, and a record less than seven days old is not asked for again. None of it reaches the public page, a public download, OAI-PMH or a frozen version; the owner's own JSON export and account export keep it.

- **Guides lead to the product.** Bing's report of 2026-09-15 put the site's reach where nobody had looked: Copilot cited SigmaCV about 340 times a day in September, three quarters of it on three guides (how to write an academic CV, its Chinese version, how to list publications), against about five Bing clicks a day and tool pages cited a handful of times. A reader arriving on a guide from an AI answer found no way to try the product short of the header. Every guide now ends with the "see it first" box (name or ORCID iD, no account) between the FAQ and the related links, under a prompt of its own in ten locales — "Now see the CV the public record builds for you". The homepage's "before you sign in" prompt stays where it has a sign-in card to sit beside.
- **Public CV pages always carry a search description.** A page whose owner wrote neither a headline nor a summary had no `<meta name="description">` at all (three live pages), and a one-word headline ("PharmD") was the whole snippet (two more) — so search engines wrote their own from the page body, usually the contact line. The description now falls back to a localized line built from what the page already shows — the name, then the short headline the owner did write or the latest visible position: "Basile Chrétien, Assistant Professor, Nagoya University — academic CV on SigmaCV, built from open research data (ORCID, OpenAlex)." Headlines of 25 characters or more stand alone as before; no gated field is ever used.
- **Six more funders' open-access policies, and NIH confirmed.** The worklist's funding section said "SigmaCV has no policy record" for researchers funded by the SNSF, Formas, FCT, the DFG, JSPS or JST; each is now recorded from the funder's own site, read on 2026-09-15 — the SNSF (no embargo for articles from projects submitted from 2023, CC BY, rights retention), Formas (immediate open access for calls from 2021, CC BY), FCT (no embargo for funding whose application period ends after 7 February 2025), the DFG (open access requested, not required), JSPS (KAKENHI calls from fiscal 2025: papers and underlying data in a repository immediately after publication) and JST (peer-reviewed articles and their data from projects in 2025 calls in its Strategic Basic Research Programs and FOREST: posted immediately; otherwise articles within 12 months). NIH's entry, which NIH's sites had kept from automated reading, was confirmed in a browser and now links the page its old address redirects to. AMED stays out: no open-access policy page was found on its own site.

### Changed

- **The Open access tab lists only the papers you can deposit today, and says why.** It listed every closed journal article — under a running embargo, with no policy known — with the publisher's record and the full statutory text under each. It now lists a paper only when a ground exists today: the publisher's recorded permission with its embargo over (the record's end date, or its duration counted from the paper's date), or a statutory secondary-publication right whose delay has run (France, Germany, Austria and Belgium after twelve months, the Netherlands after six, Bulgaria on acceptance) — a rule of law a publisher's terms do not remove. Each row is the title, the one action with Copy DOI, and one sentence naming the ground ("Allowed by the publisher's policy: accepted manuscript, as OA.Works recorded on 2021-01-27. The embargo ended on 2022-06-24." / "Allowed by law — Code de la recherche, art. L533-4 (France): the accepted manuscript, 12 months after publication (since 2024-12-31), under the conditions in the record below."); the record, the statutory text and the form notes stay behind the row's disclosure. The state chip, the two help paragraphs and the Search-indexing / Institution-listing status rows are gone from the tab (both choices stay in the Publish menu and their prompts). The chips on the publication rows follow the same list.
- **The German and Austrian secondary-publication lines are dated.** Germany's § 38 Abs. 4 UrhG has been in force since 1 January 2014 and Austria's § 37a since 1 October 2015; whether either reaches a contribution published earlier is disputed (Germany) or unaddressed (Austria), so — as for Bulgaria, Spain and Japan — the line is no longer shown under a work from an earlier year, and each line now says since when the right exists.
- **Japan's open-access policy is read from the Cabinet Office's own documents.** The worklist's Japan line pointed to a university library's English explanation. It now links the Cabinet Office's basic policy of 16 February 2024 — labelled "policy text", a kind beside "legal text" and "guidance" — and its implementation measures (21 February 2024, revised 8 October 2024), and its statements follow those texts: projects from calls newly opened from FY2025 (JSPS KAKENHI, JST Strategic Basic Research except ALCA-Next and CRONOS, AMED's advanced R&D support, JST FOREST); the publisher's version or the accepted manuscript, in the institution's repository or another platform findable through NII Research Data Cloud; "immediate" means no embargo, with no fixed period but about three months to being public as the guide; and where a journal's embargo prevents it, the reason is reported to the funder and the deposit made promptly once it lifts. The line now appears under journal articles only, as the policy covers papers in e-journals.
- **A prose section holds 20,000 characters, not 8,000.** The cap on a narrative section's body was sized before any funder format with a six-page limit and no word limit existed in the catalog: at roughly 3,000 to 3,500 characters a page, the FRQ contributions section alone can exceed 8,000, and the editor cut applicants off mid-sentence with no way round it. The cap is now 20,000 per section, the schema, the editor counter and the published JSON Schema all reading the one constant.

### Fixed

- **The data-links lookup cannot be held up by one failing work either.** The Europe PMC pass counts a work as checked only when every one of its lookups completed, and ordered its per-sync budget by that: a work whose data-links call failed on every sync stayed first in the queue and the rest of the CV was never reached — the stall just fixed in the publisher-policy pass. Every examined work is now stamped as tried, a failed or circuit-broken lookup included, and the rotation orders on that; a lookup that did not complete is still asked again, behind the works waiting for their first one.

- **The publisher-policy lookup no longer stalls on one slow DOI.** In the owner's sync, the OA.Works pass asks about each closed journal article in turn under a wall-clock budget, and a work whose call failed was left unstamped so it would be asked again first. A DOI whose record OA.Works computes slowly times out on every call, so two of them spent the whole budget on every sync and the rest of the CV was never asked about — on the live site, 64 of 3,285 closed articles had ever been checked. The pass now stamps every work it examines as tried (the policy and its date still come only from an answer), so a timing-out DOI queues behind the works waiting for their first lookup — and a work answered long ago that starts failing keeps moving back rather than holding the queue on its old answer date.

- **A clicked paper's DOI no longer reaches the analytics collector.** The site's Plausible script tracks outbound-link clicks, and its event carries the clicked URL — so a click on the owner worklist's ShareYourPaper link (`shareyourpaper.org/<DOI>`), or on any `doi.org` link, sent the paper's DOI to the collector. The init script's `transformRequest`, which already strips the looked-up iD from preview paths and the typed name from the lookup, now keeps only the origin of the URL an event carries before the request leaves the browser (`https://doi.org`): clicks still count by destination, and no identifier in a path — a DOI, an ORCID iD, an OpenAlex id — crosses the wire, whatever link a later change adds.
- **Two guide titles over Bing's 100-character `<title>` ceiling.** The French and German "responsible metrics" guides ran to 105 and 100 characters with the layout's " — SigmaCV" suffix (Bing's one "title too long" finding); now 90 and 99 with the suffix (the German one keeps "Leiden Manifesto" in full — a bare "Leiden" reads as the common noun), and a test now holds every guide title in every locale under the ceiling.

- **Homepage: the first thirty seconds.** The headline said the iD was the prerequisite ("from one ORCID iD") above a box that takes a name; it now leads with the outcome and the no-login path — "Your academic CV, built from the public record. Type your name to see it." The mission eyebrow ("Open infrastructure for responsible research assessment") is for funders and librarians, so it moves down to head the Why section; the hero's eyebrow now says what a visitor needs first: free, open source, no account needed to look. The sixty-word subtitle that listed five funders and four formats is two sentences (the funder names still live in the layouts section). The card loses three of its nine pieces: the box's hint is one line, and the two disclosures under the sign-in button — what we access, and what an ORCID iD is — become one, "How sign-in works". On phones the order is headline, box, subtitle, then the graphic, so the box is on the first screen instead of two screens down. Ten locales; the social-card image follows the new copy.
- **Lookup rows: the most recent record stands for the person, on two lines, without a rank.** OpenAlex keeps several author records for one iD, and the first-ranked one could be a decades-old institute — a searcher looking for themselves read "National Institutes of Health · 1975" beside their own name and concluded it was someone else. Among split records the row now keeps its place in OpenAlex's order but shows the affiliation with the most recent year (a record with no dated affiliation never displaces one that has one). Each row is a card: name on the first line, affiliation, years and the ORCID mark on the second, so they no longer run together; the list is unordered, because a numbered list puts a rank beside every name and the page promises none. The search box keeps what the visitor typed, with their casing, instead of echoing the lower-cased query the lookup ran.
- **Homepage: "see it first" is the front door, and it takes a name.** The sign-in card led with OAuth and kept the no-login preview as an afterthought under the buttons, and that preview took only an ORCID iD — the one thing a cold visitor is least likely to have to hand. The card now opens with one box (`SeeItFirstForm`, ten locales), headed "See what SigmaCV finds — before you sign in": an ORCID iD, bare or as an orcid.org URL, opens the automatic preview at `/preview/<iD>`; a name opens the lookup at `/search?q=` with the query carried over, under the same hygiene the lookup applies server-side (`src/lib/openalex/authorQuery.ts`, pure and client-safe, so the OpenAlex client never reaches the browser); an iD-shaped string with a bad checksum, or anything shorter than three characters, is refused in place with the reason, and focus returns to the field. The box never calls the machine build "your CV" — the preview's banner says it may hold a namesake's work, and a name lands on a list of people — so the button is a plain "Show me" and the hint says a name lists researchers who have an ORCID iD. The sign-in follows under a rule with one conditional line — "Is it yours? Sign in to curate, export and publish it." — and the quiet "Looking for someone else?" entry stays at the bottom of the card (both name paths reach the same `/search`; they differ in who is asking). The same box replaces the iD-only form on `/orcid-to-cv` and, headed as a retry, on the preview's malformed-iD notice. The lookup now shows one row per ORCID iD when OpenAlex holds several author records for one person (the first-ranked record's affiliation stands). Two cookieless events are added: `See it first` carries only whether an iD or a name was used, never the value; `Sign-in error` carries our own mapped bucket, never Auth.js's raw `?error`, so sign-in failures stop being invisible (Plausible had shown 21 visitors on that page in six months with no way to learn why). And because the box will send many more typed names through `/search?q=`, the Plausible pageview scrub now drops that query string before the request leaves the browser, as it already did for `/preview/<iD>`.
- **Institution pages and the comparison view: what the last full year stood at one reading earlier.** When the weekly refresh stores a new OpenAlex snapshot, the outgoing one moves into two new columns (`openalexPreviousAggregates`, `openalexPreviousFetchedAt`; additive migration; cleared with the snapshot when a ROR leaves the opted-in sets). The institution page's OpenAlex section, and each column of the comparison, then print one line — "At the previous reading (date), 2025 stood at 640 / 1,000 = 64%" — the honest stability signal the panel of 2026-09-10 asked for: both readings visible, never a difference, never an arrow, and nothing at all when either reading states no share for that year (a partial year at a year boundary, or below the floor), when the year is not among the years the compared columns share, or when the comparison's columns are skewed. The JSON export carries the earlier reading as counts only. Ten locales.
- **Comparison view: the counts-only JSON export (`/i/compare.json`, PR5 of the comparison programme).** The same organisations, years and counts the page shows — works with a status, open copies, none found, the status buckets, the field mix, each organisation's ROR and OpenAlex identifiers, folded entities, counted work types and snapshot date — and nothing the page would refuse to state: no share, no percent, no difference; each row says why the page states no share for it (`notStatedBecause`: partial year, below the floor, snapshots read too far apart) so a reuser can honour the same rules. Released under CC0 with a citation request carried in the envelope (OpenAlex, arXiv:2205.01833, and the method version `sigmacv-institution-comparison/v1`), readable cross-origin, never indexed, never cached, and behind the same rate limit as the page (now shared in `compareLimits.ts`); fewer than two organisations to set side by side is a 404 that names the ids left out and why (as the page does, with the page's three reasons stated separately), and a value that is not a ROR id is never echoed. The envelope also carries the page's own thresholds (the floor, the skew window) and a one-line meaning for each reason. The page links to it under the columns; the terms say the export is public data under CC0, which places no condition on its reuse, and that SigmaCV asks — without requiring it — that reusers cite OpenAlex and SigmaCV and do not republish it as a ranking (the owner's choice of 2026-09-10: a request, never a licence condition).
- **Comparison view: trajectories, status breakdown, field mix, and the way in (PR4 of the comparison programme).** Each column of `/i/compare` gains one small figure of its own — the organisation's stated open shares over the common full years on a fixed 0–100 axis, drawn in the text colour, a withheld year a gap and nothing drawn when no share is stated; never two lines on one plot, never a bar against another column's, and named for screen readers — then a second table splitting the open copies by OpenAlex status (gold, hybrid, diamond, green, bronze; `closed` stays the "none found" column above), and, for a row refreshed since #457, the field-mix table with its own total. The organisation's page gains its one way in: "Set beside another organisation" under the OpenAlex section, opening the picker with that organisation pre-ticked. Ten locales; the compare page's vocabulary ban covers the new copy.
- **Institution comparison view: two or three organisations' OpenAlex records side by side, never ranked (`/i/compare`, ten locales).** Designed by a five-lens panel on 2026-09-10 before any code, after the owner chose it on 2026-09-09. The page sets side by side the stored weekly snapshots of up to three organisations (`?ror=a&ror=b[&ror=c]`; the fourth and beyond are left out and said so), each in its own column with its own table — the full years every record covers, then that column's own incomplete year — each row `total (works with a status) | open copy | none found | open share`, the share in one cell with both of its counts under the floor and current-year rules of #456, and each column carrying its own as-of date, counted OpenAlex entity with the number of folded organisations, and country. Structure is the argument: columns are ALPHABETICAL whatever order the URL asked (a shared link cannot encode "X before Y"), the canonical URL is the sorted set of ids, the method box and the visible "not adjusted for field, language, publisher or size" label come BEFORE the numbers, there is no sort, no difference, no chart, no third column SigmaCV chose, no listed count, nothing from the consented figures, and when the snapshots were read more than 30 days apart every share is withheld ("records read too far apart") and the counts stand. Only an organisation under which at least one researcher chose to be listed (page existence — the count itself never reaches the page), whose snapshot parses and which has at least one full year over the floor can be set side by side; an id with nothing to show gets one neutral line, an id that is not ROR-shaped is never echoed. With fewer than two, a plain GET picker lists the organisations that can be set side by side (nothing pre-ticked beyond the URL). The page is `noindex, follow`, has no hreflang and is absent from the sitemap (a set of near-duplicate "X beside Y" pages is a crawl trap and the league-table artefact the project refuses). It checks its own bucket (20 per IP per minute) and then charges the public-page limit once per accepted organisation (at most three). Copy: a method box (what is counted, folding never parents, affiliations are publisher strings parsed by OpenAlex, "none found" ≠ none exists, the share's definition and floor, Method v1), nine caveats (field mix, size, current year, embargo lag, "none found" ≠ paywalled, folding, the self-selected universe, weekly drift, and — only when the columns are in different countries — national policy), and a disclaimer with the contact address; ja/zh say 並置 / 并列, never 比較 / 比较, and a test bans rank, verdict, comparative and compliance vocabulary in every locale. The index intro no longer says "never a comparison between institutions"; it says records can be set side by side, never ranked. The terms gain one sentence (a description, not an assessment; no republishing as a ranking, no automated decisions, no bulk extraction) and the privacy notice one (counts of works, never people; nothing about the listed researchers is used, shown or compared). Both invariant suites — the source grep and the offline render with `fetch` torn out — cover the new module, component and routes.
- **Institution pages: the field mix, so a share is read against what an organisation publishes.** The weekly OpenAlex snapshot gains one more grouped request per organisation (`group_by=primary_topic.domain.id`, verified live: four domains — Life, Social, Physical and Health Sciences — keyed by OpenAlex's domain ids; 12 polite-pool calls per row now — one entity lookup and 11 grouped requests), stored as counts with that request's own total under an OPTIONAL `domains` field of the counts-only snapshot (rows written before it still parse and simply show nothing until their next weekly refresh). The page renders it as one small table, "OpenAlex domains": works by the domain of their primary topic over the window, in OpenAlex's own domain order so two pages line up row for row (never sorted by count), with the total stated in the note (the rows can add up to fewer: a work OpenAlex has given no topic is in no row). The note says what the panel of 2026-09-10 asked it to say: the open share is not adjusted for this mix, and where two organisations differ here their shares do not measure the same thing. Counts only — never a field-normalised share, never a share of the mix; ten locales, under the same vocabulary ban as the share copy.
- **Institution pages: the open share, stated beside its counts.** The "Open-access status by year" table on `/i/<ror>` gains one column: the works OpenAlex records under any status but closed (bronze included), divided by that year's number of works with a status, stated as a whole percent in ONE cell with both counts (`400 / 1,230 = 33%`) — the one figure the institution pages derive from OpenAlex's counts (the owner's decision of 2026-09-09; designed by a five-lens panel on 2026-09-10 before any code). The share is computed at render from the stored counts (`src/lib/institutions/oaShare.ts`; the snapshot stays counts-only), and it is withheld, with the reason in the cell, for the snapshot's current year ("year not complete": the year is not over) and for any year below 100 works with a status ("too few works to state a share": at 100, one work is one printed point). The table's Total is now the sum of its own status columns — the same OpenAlex request, so the columns always add up and the share is bounded 0–100 % — rather than the works-by-year count from another request; the note says so, and the page names the years where the two differ by more than one percent. A note above the table states the definition, the floor, the current-year rule (and that the year before it may still move as embargoes end), and that the figure is not adjusted for field, language, publisher or size, so a few points' difference, or a shift between records read weeks apart, means nothing on its own. No sort, no difference between years, no colour, no axis; ten locales, with a test that the share copy carries no rank, verdict, comparative or compliance vocabulary in any of them.
- **Indexing as an active choice at publish.** Search indexing stays off by default, but it is no longer decided by silence: once a page is live and not yet indexable, one inline question — "Should search engines index your page?" — is put in front of the owner (ten locales), with what a yes does (search engines, and the OAI-PMH harvest an indexable page allows), one sentence that the institution listing is a separate choice asked next, and two equal buttons: "Yes" (one request that turns indexing on and touches nothing else) and "Not now" (sends nothing; remembered per page for as long as that page exists — the worklist keeps the choice reachable). It is an onboarding step ahead of the institution ask, never stacked with another prompt, and the worklist gains a "Search indexing" status row (not decided / off / on) with the same one-request control, so the open choice stays visible after a "Not now". Nothing is pre-ticked anywhere. A Playwright journey covers publish → question → not now remembered → yes.
- **Homepage: a quiet entry to the lookup.** At the bottom of the sign-in card, one line for the visitor who is not the owner — "Looking for someone else? Find a researcher by name" (ten locales) — linking `/search`. The hero and the sign-in call to action are unchanged.
- **Find a researcher by name (`/search`, ten locales).** The lookup's promise is on the page — "Look up what a researcher has published — not how they score" — and the page keeps it: a query goes to OpenAlex's own author search (`search=`, never the filter grammar, so a visitor's string can never become filter clauses under our polite-pool identity), restricted to researchers with an ORCID iD, ten rows in OpenAlex's order; a row is a name, the affiliation OpenAlex publishes with its years, and the ORCID mark — no works count, no citation count, no sort control — and leads (`nofollow`) to the automatic preview, which is where the third-party banner, the no-figure rule and the objection route already live. Only the bare page is indexable (and in the sitemap); a result page is `noindex`: a list assembled for one visitor's query is never a crawlable index of people. Cost control for a shared quota: its own rate buckets (20 per IP per minute; 300 per minute and 3,000 per hour globally — never the preview's), a bounded in-process cache of positive and negative answers, one attempt with a short timeout, and `no-store` for Next's data cache (a new `cache` option on `resilientFetch`), so an attacker-chosen query cannot write a disk entry. Queries are NFC-normalised, bounded to 3–80 characters and refused outright if they carry `, | ! : < > " \` or control characters. The privacy notice discloses the lookup beside the preview. A Playwright journey covers the round trip from name to preview.
- **A self-service objection route for people who never used SigmaCV.** The privacy notice promised that anyone could object to the no-login preview of their ORCID iD by email; now they can do it themselves at `/object` (ten locales): the page explains what objecting does and does not do, then one clearly labelled button sends them to ORCID once — through a second provider id on the same ORCID app, so the round trip is bound to that page and nothing else — and on the way back Auth.js's sign-in callback records the objection and **aborts the sign-in**: no account, no session, only `HMAC-SHA256(PREVIEW_SUPPRESSION_KEY, iD)` in a new `PreviewSuppression` table (additive migration; independent of `User`, so it survives an account's deletion and can pre-date the account). A suppressed iD is answered exactly like an unknown one — the empty notice, before any source is queried, cache dropped at once. Account holders get the same setting as a checkbox in the account menu ("Do not show an automatic preview of my record"), which also lets them switch it back on; a non-user who changes their mind signs in or emails. The landing page after the round trip carries the outcome only, never the iD, and is not indexed. The email route stays as the fallback, and the PR C env list still works for it. Go-live: register the second redirect URI (`/api/auth/callback/orcid-object`) on the ORCID app (CLAUDE.md checklist).
- **Institution listing: an active choice, put in front of you once — never a default.** The three institution consents stay off by default (a pre-ticked box is not consent), but they were easy to miss. Now, when your public page is live and indexable and a current position is linked to a ROR record, the editor shows an inline card once: "List yourself under {institution}?" with the disclosure (the OAI-PMH set for repositories; the institution page and its figures; nothing is listed until you choose; absence means nothing; withdrawal is immediate), two buttons of equal weight — "Yes, list me under {institution}" and "Not now" — and a "What this means" link to the privacy notice. "Yes" is one request carrying your full current publish state; with several current institutions a picker appears, nothing pre-ticked, and "Yes" stays disabled until you tick one — never two institutions in one click. "Not now" sends nothing and is remembered per set of ROR ids, so a new affiliation asks again; a later withdrawal in the Publish menu is remembered the same way. The owner worklist gains a first status line, "Institution listing" — "You are not yet listed under …" with the same "List me under …" button, "Listed under …" with a Change link, and the existing wording for a lapsed consent — which never counts as a gap in any figure. The Publish menu's three institution boxes now sit under a "Your institution" sub-heading that the Change link opens. The two consents are two SURFACES and are never collapsed into one: someone already on their institution's page but not in its repository set (or the reverse) is told exactly that, with a Change link, and is not asked again. The worklist's "List me under …" button now carries the same one-sentence disclosure and "What this means" link as the card, naming the institution whose repository set the listing would join. Withdrawing the last institution — from the toggle, the picker, or by removing the last kept id — is remembered as an answer, so the card does not come back. The anonymous preview never shows either; the privacy notice is unchanged; the copy is test-enforced across the ten locales (no compliance words, no mandate, no percentage or ratio, nothing that implies the institution asked).
- **Owner worklist: "Your grants and their open-access policies" — the funder join, owner-only, facts side by side.** A work's printed funders are often a co-author's, so the only funder statement SigmaCV makes is about the owner's OWN grants, and it makes it in the editor worklist alone. For each work that acknowledges one of the grants in the owner's Grants section, one row: the work, "acknowledges award X from Y" (or, when the work names the funder without an award number, the weaker "names your funder Y; no award number on the work"), the funder's recorded open-access policy with a link to the policy on the funder's own domain (otherwise "SigmaCV has no policy record for Y."), then "SigmaCV found: <one of the worklist's four open-access states>" and the existing Open Policy Finder link. Never a verdict, never a colour that says good or bad, never a count of works "needing action" (the heading carries no number), and the vocabulary is enforced across the ten locales (compliant, non-compliant, overdue, violation and now mandate).
  - **The join is conservative and pure.** A grant's funder id (ORCID's FUNDREF / ROR / GRID, a Crossref FundRef DOI, or a borrowed OpenAlex id) and a work's OpenAlex funder id live in different namespaces and are never compared as strings: a new `Funder` crosswalk table (additive migration; OpenAlex `/funders`: FundRef DOI, ROR id, Wikidata item per funder) resolves both to the same OpenAlex funder. An award number matches only after normalisation of case, whitespace, hyphens and slashes (leading zeros kept, three characters minimum) and never when a comparable id disagrees — the grant's own FundRef DOI or ROR id against the one OpenAlex records for the work's funder, or the OpenAlex id the grant carries or resolves to against the work's — so a grant whose funder is missing from the crosswalk is not thereby a grant whose funder agrees; only when neither side offers a comparable id does the award number decide alone; a GRID or RINGGOLD id counts as no id; a work whose award number did not match is never joined by its funder; hidden grants and hidden or "not mine" works never join; output order is deterministic. The crosswalk is written by the authenticated sync only, AFTER the owner's document is saved so the write never waits on it: at most 25 polite-pool calls per sync, each a single attempt with its own 10 s timeout and no retry, and no new lookup starts after 10 s; stalest first, refreshed every 90 days, fail-soft, ~0 calls once warm; the anonymous preview never writes it (tested).
  - **The funder open-access policy table (`src/lib/funders/oaPolicies.ts`) is committed, hand-written data keyed by FundRef DOI** — ten funders whose policy is public and stable (UKRI, Wellcome, NIH, NSF, ERC, European Commission / Horizon Europe, ANR, Gates Foundation, FWF, NWO), each with the policy URL on the funder's own domain, short factual statements, and an effective date only where the funder states one (a funder that scopes its policy by call carries the call wording instead of an invented January-1 date). Every entry is currently **pending live confirmation** — written from memory, with no verification date — and the owner sentence says so ("drafted from memory and not yet confirmed against the funder's site: …"); only an entry a maintainer has confirmed carries `lastVerified` and reads "as recorded on <date>" (the pairing is enforced by test). The pending entries are listed in `docs/FUNDER-OA-POLICIES.md` for the maintainer to check before deploy; the test fails if one is missing there, or if any field says compliant, non-compliant, overdue or mandate.
  - **Nothing of this is public.** `meta.funders` stays stripped from every public surface, and a source-level invariant test asserts that no public route, the OAI endpoint, the public JSON-LD / projection / formats, the renderers, the snapshot code, or the anonymous preview (route, workspace, builder) imports anything from `src/lib/funders/` or the crosswalk writer.
- **Institution pages: a per-researcher reconciliation export (`/i/<ror>/reconciliation.csv` and `.json`), behind a SECOND opt-in and a researcher-designated frozen version.** An institution reconciling its records (CRIS, repository, funder reports) needs, per researcher who chooses to help, a stable machine-readable list of "these are my works, this is the open-access state SigmaCV found, this is my affiliation" — endorsed by the researcher, versioned, and free of any verdict. This is the programme's ONLY per-person surface, and it exists because the researcher opts in a second time, designates the exact frozen version, and can withdraw at any moment. (1) **The second opt-in** — a new `Cv.shareReconciliationRows` column (additive migration) offered in the Publish panel only while "Show me on my institution's public page" is stored (not merely armed), posted on its own, and cleared in the same write whenever that consent is withdrawn — explicitly, with indexing, or on unpublish; the publish route accepts `shareReconciliationRows?: boolean`. Its copy, in ten locales, says what is shared (your ORCID iD; the works your public page lists with their DOI, year, type, licence and the open-access state found; the institutions you ticked), where (a public CSV/JSON under `/i/<ROR id>/reconciliation`), from which version (the frozen version you designate — nothing is shared until you designate one, nothing changes until you designate another) and how to stop (untick; the rows disappear with the next request); the privacy notice gains the matching sentence, and the GDPR export carries both new columns. (2) **The designated frozen version** — a new `CvSnapshot.forReconciliation` column, set from the Versions panel ("Use this frozen version for my institution's reconciliation export", with a tag on the designated row): only a public version can be designated (a private one's action is disabled with the reason, and the server answers 409 `not-public`), at most one per CV (designating another clears the previous in one transaction), making it private drops the designation, and deleting it drops it with the row. Designating computes the export's rows from the frozen document ONCE, right there, and stores them beside the flag in a new `CvSnapshot.reconciliationRows` column (additive migration; undesignating clears them; the same migration adds a partial unique index on `(cvId) WHERE forReconciliation`, so "at most one designated version per CV" is a database fact even under two concurrent designations), and either change — as does deleting the designated version — purges the institution pages this CV is consented to, so the page's line moves at once rather than within the cache TTL. (3) **The rows** — a pure module (`reconciliationRows.ts`) reads the frozen document through `freezeCanonical` and the public projection — never the live document, and only at designation — and emits EXACTLY fourteen columns: `orcid, ror_ids` (the consented ids still current, semicolon-separated — a lapsed id never rides a row), `item_id, doi, title, year, section_type, oa_state` (the owner worklist's four states: `open-cc | open-other | no-open-copy-found | not-determined`), `license, oa_url, retracted, snapshot_version, content_hash, frozen_at`. The work set is the public page's own list, now exposed as ONE shared predicate (`listedWorks` in `cvAggregates.ts`) that the per-CV aggregate uses too: a work absent from the page is absent from the export; a retracted work the page still shows is carried with its flag (the counts and the OAI-PMH records leave it out). No column carries a verdict, a percentage, a funder, a citation count, or anything from `meta` beyond the listed fields — a test greps the header and every cell of the CSV and the JSON for `compliant`, `non-compliant`, `overdue`, `%`, `share` and `rate`. A CSV cell a spreadsheet would evaluate (one starting with `=`, `+`, `-`, `@`, a tab or a carriage return — OWASP CSV injection) is prefixed with a single quote; the JSON row is untouched. (4) **The reader** (`reconciliationSources` in `listed.ts`) filters on seven columns in SQL — the page consent, the second opt-in, published + indexable, the id among the consented ids AND among the visible current positions (the lapse rule), an owner with an ORCID iD, and a designated public version WITH stored rows — selects those stored rows and the owner's ORCID iD from the User relation (the page itself still never reads it), never selects the live `document` nor the frozen one (no public request parses a document: a stored value is validated on read, and one that no longer parses is skipped and not counted as a contributor), and is bounded (first 1,000 CVs by id; the envelope says `truncated`). The page's count uses the same clause, so its line and the export's `contributorCount` agree. (5) **The routes** validate the ROR id by shape, share the public pages' rate limit, 404 a ROR nobody is listed under (the page's known-miss rule; a page with nobody sharing rows gets an EMPTY export instead), take no parameter beyond the id (the plan's "Institution input" veto — a query string changes nothing), and answer `Content-Disposition: attachment; filename="sigmacv-reconciliation-<ror>.csv|json"`, `Cache-Control: no-store`, `X-Robots-Tag: noindex`. The CSV is RFC 4180 (header first, every field quoted, inner quotes doubled, CRLF records, an embedded newline stays inside its field, no BOM); the JSON is `{ ror, generatedAt, rowCount, contributorCount, truncated, rows }` — no aggregate, no ratio, no comparison with OpenAlex, no "unclaimed" row. Both invariant suites (source-level: no external client, no `fetch`; runtime: rendered with `fetch` stubbed to throw and `@/lib/http` torn out) now cover the new modules and both route handlers. (6) **The page** gains one line under "Machine access", shown from the first contributor (each row is a named, opted-in researcher, so there is no k here): "N researchers share their reconciliation rows: download CSV · JSON", and the "About this page" block then says that rows in the export identify the researcher by ORCID iD because each one chose to share them; the page itself stays roster-free. Ten locales throughout, with a per-locale vocabulary test (compliance, overdue, percentage, share as a noun, rate, ratio, mandate, and their zh/ja/ko/ru/de/fr/es/it/pt equivalents) over every new string. Deleting the account removes the researcher from the export at once (the snapshot cascade; tested against the delete route).
- **Institution pages: "Figures from researchers who chose to be counted here" — the researchers' own listed works, summed under k-anonymity (k = 5 per cell, complement suppression).** Each `/i/<ror>` page gains a second, separate count beside the existing "N researchers list this affiliation" (the repository-listing opt-in): how many researchers gave the PINNED institution-page consent for that ROR and whose consent is still active (a consented id no longer among the visible current positions is lapsed and not counted), each stated as its own sentence, never as a ratio of one to the other. Their figures are computed per CV at every write — sync, save and publish-state change, beside the OAI set key — into a new `Cv.institutionAggregates` column (additive migration; rows written before this change carry null until their next save or the weekly re-sync, and the page says how many are "not yet computed"; no backfill job) as COUNTS of exactly the works the public page and the OAI-PMH per-work records list (same projection, same selection, retracted works out; hidden, "not mine" and view-excluded works never enter), by year × the owner worklist's four open-access states (open with a Creative Commons licence / open with another or unknown licence / no open copy found / not determined — labels reused, not redefined) and by section; a work with no year is filed under "No year". The page sums them and shows a works-by-year table (rows = years, the year's total as the stated denominator, then the four states) and a works-by-section table, under four suppression rules in order, whose unit is the SET of CVs a figure comes from: a year row needs 5 contributing CVs; a cell nobody has a work in is a plain 0 (it discloses nobody), a cell 1–4 CVs contribute to reads "fewer than 5 researchers", and one 5 or more contribute to is shown; within a row whose total is shown, while the hidden cells together stand on fewer than 5 CVs the smallest shown cell is hidden with them, so the total minus the shown cells is only ever the sum of a hidden set that 5 CVs stand behind — never one or two CVs' figure, not even as the sum of two hidden cells; and across the two tables — both of which add up to the same overall total, so the shown sections minus the shown years always equals the hidden years minus the hidden sections, whether one table is fully shown or both are partially hidden — whenever the hidden year rows and hidden sections together stand on fewer than 5 CVs, the smallest shown entry of either table (year rows by their total, sections by their count; a tie goes to the year table) is hidden too, until the hidden set stands on 5 CVs, so a single CV's hidden year can never be read as the sections' total minus the shown years (or the reverse), nor one CV's hidden year minus another's hidden section as the difference between the two tables. A property test over 600 random pages asserts all four: every shown number has 5 contributors or is a structural 0, and every recoverable difference is the sum of a hidden set 5 CVs stand behind. Below 5 contributors the section is one sentence: "Fewer than 5 researchers have chosen to be counted here, so no figures are shown yet."; from 5 contributors with every figure suppressed, one sentence says so instead of two empty tables. No chart, no percentage, no share, no name, no per-person column and no link to any CV from the section; the Organization JSON-LD is unchanged (no `employee`/`member`, no numbers). The OpenAlex snapshot section and this one never meet: neither module imports or names the other, the page hands each only its own data, and a sentence states that the researchers' figures are not compared with OpenAlex's record and neither is subtracted from the other (tested at the source level). The reader is bounded (first 2,000 consented CVs by id, stated on the page when more exist) and reads Postgres only — five columns in SQL, the stored aggregate the only thing selected: the "consent still current" rule is a new `Cv.visibleCurrentRorIds` column (the bare ROR ids of all visible current positions, rewritten from the document on every write beside the OAI key; rows written before it carry `[]` until their next write, and are not counted until then), so no candidate document is ever parsed on a public request — both the source-level and the offline-render invariant suites cover the new module, reader and component. The column is derived from the owner's own data, so the GDPR export carries it; account deletion already drops the row. Copy in ten locales, with a test that none of it, in any locale, says compliant, non-compliant, overdue, share, ratio, rate or percentage.
- **Institution pages: "OpenAlex's record of this organisation", counts only, from a weekly snapshot.** Each `/i/<ror>` page gains a section rendered from a snapshot the internal resync tick stores on the `Institution` row (`openalexId`, `openalexAggregates`, `openalexFetchedAt`, `openalexLastError`, `openalexNextRefreshAt`) — never fetched when the page is opened: a request still reads Postgres only, and both the source-level and the offline-render invariant tests now cover the new module and component. The job refreshes at most 20 institutions per tick (~10 polite-pool calls each, paced, 60 s wall-clock budget, its own 5-minute guard), stalest first, only for ROR ids at least one researcher opted to be listed under; a failure is recorded on the row and retried a day later; a ROR that leaves the opted-in set keeps its trusted name but loses its OpenAlex columns. What the section shows: the counted entity ("counted as OpenAlex entity I…, lineage n, m associated organisations" — hospitals and labs that OpenAlex records as related or child institutions are folded in, since a CHU is a separate entity from its university, parents are not), a works-by-year table and an open-access-status-by-year table over the last six full years plus the current one (rows = years, columns = OpenAlex's own statuses, the year's total as the stated denominator), the top-15 co-author countries and top-15 co-affiliated organisations (the institution's own lineage and associated organisations excluded), and "as of <date>". Every series counts articles, reviews, book chapters and preprints only and says so — datasets swamp the current year (Nagoya shows 91,924 "works" for 2026). No ratio, no percentage, no share axis anywhere in the stored JSON or on the page (tested; on the page, superseded by the open share — see above), and a sentence states that these are OpenAlex's figures about the organisation, not about the listed researchers, and that the two are not compared. The trusted ROR name stays the page's name; OpenAlex's `display_name` appears only as counted-entity context. The Organization JSON-LD gains `sameAs: https://openalex.org/I…` when known. Copy in ten locales; "OpenAlex figures not fetched yet." until the first pass.
- **Wellcome's open-access policy is confirmed on its own site.** The funder policy table was drafted from memory, and its Wellcome entry pointed at an address that now answers 404 and paraphrased the 2021 policy. Read on wellcome.org on 2026-09-15 (the page says it was last updated on 2 January 2025), the entry now carries the current page, the date it was read and the policy as the page states it — Europe PMC on publication, CC BY (CC BY-ND by exception), the Version of Record or the accepted manuscript, a data availability statement — so the worklist's funding section says "as recorded on" for it. The same reading confirms Wellcome's deposit route: Europe PMC plus, for a self-archived accepted manuscript, is now the one action offered under a closed article that names Wellcome.
- **The funder open-access policy table is checked against the funders' own sites.** Every entry had been drafted from memory, and the worklist's funding section said so for each funder. On 2026-09-15 eight more were read on the funder's own pages and now say "as recorded on": UKRI, NSF, the ERC, the European Commission (whose terms are on its research executive agency's page), ANR, the Gates Foundation, FWF and NWO. Four addresses moved to the page that actually states the terms, and what the pages do not say was dropped or corrected — NSF states no version or embargo, the Commission's page does not say "immediate", ANR's terms apply to projects funded from 2022 and FWF's to projects approved after 31 December 2020 (not "calls"), NSF's deposit rule applies to awards from proposals submitted or due on or after 25 January 2016, and Gates asks for funded manuscripts immediately under CC BY 4.0, a preprint as soon as feasible, deposit in PubMed Central or another open repository and open underlying data, and pays no article processing charges. NIH's sites put a bot check in front of automated readers, so its entry stays marked as not yet confirmed.

### Fixed

- **The no-login preview shows no figure about the person, and says who it is for.** A visitor could open `/preview/<ORCID>` for any researcher and switch on that person's h-index, citation counts, per-work FWCI/RCR pills and a most-cited sort — a number on someone who never signed in and never chose it. The preview projection now strips the author-level metrics, the per-year series and every per-work indicator (citation counts, FWCI, top-decile, RCR, clinical citations, APT, the OpenCitations count), forces the corresponding display toggles off and resets a citation sort, and the anonymous render endpoint (`/api/preview/render`) re-applies that projection to whatever document the client posts, so no render can show a figure whatever is toggled — or forged in a request; the anonymous editor hides the metrics group and those toggles too (`anonymous` now reaches `StyleControls`). The privacy notice states the promise in ten locales. The page is framed for a third party: a banner says this is an automatic, unreviewed build that may include a namesake's work ("Look up what a researcher has published — not how they score") with a link to the objection route — or, when the researcher published an indexable page, points at that page instead (the sitemap's own predicate, resolved per request, never inside the cached build; a published-but-unindexed page is never revealed). Two calls to action, split: "This is my record — sign in" and "Know this researcher? Copy the link" (clipboard only, no email). Owner-voice copy on the preview ("Preview my CV", "Building your CV preview") is reworded for a visitor who may not be the owner, ten locales. Cookieless `Preview` / `Preview CTA` events carry the outcome or action only, never the iD. Two defects from the 2026-09-08 demand-side panel are fixed on the way: the provenance ledger no longer prints a percentage on recipient-facing pages ("4 of 10", not "4 of 10 (40%)" — the pure ledger keeps `share` for the owner's editor panel), and reader mode no longer forces the per-work FWCI/RCR indicators on (they exist for some fields and not others, so forcing them graded CVs by discipline). A Playwright journey covers the banner, the split CTAs, the absence of every figure control, and the published-page link appearing only once the page is indexable.
- **Privacy notice: the no-login ORCID preview is now described accurately, and people who never used SigmaCV get a stated objection route.** The notice said the preview was built "only from the public sources listed above" while naming five; the build queries seventeen, and the national funder, clinical-trial and patent registries (UKRI, NIH RePORTER, NSF, ClinicalTrials.gov, EU CTIS, WHO ICTRP, the European Patent Office) are queried by the researcher's NAME and organisation, for a person who may never have asked. The "What we process" list now names every source, says which ones are name-matched and that their results are shown only as flagged candidates; the preview paragraph states the Art. 14(5)(b) reliance (this notice is the information owed to someone who never signed up), the legitimate-interest basis, the APPI purpose of use, that the preview is discarded after a short cache and not stored as a profile, and that the analytics do not record which iD was looked up. A new paragraph under "Your rights" tells a non-user how to object (GDPR Art. 21): email with the ORCID iD, honoured within 30 days, no reason asked, nothing shown to visitors. That promise now has a mechanism: `PREVIEW_SUPPRESSED_ORCID_HMACS` (HMACs printed by `npm run preview:suppress-hash`, never plaintext iDs) makes a listed iD preview exactly like one with no public record — checked before the cache and before any source is queried. The Terms gain a "Looking up other researchers" clause (a preview is automatic, unreviewed by its subject, not an assessment or ranking by SigmaCV; no automated decisions, profiling, harassment or bulk extraction). And the analytics claim is made true: the Plausible init stub now scrubs `/preview/<ORCID>` to `/preview/_` before the pageview leaves the browser (tested in a sandbox), and `docs/ANALYTICS.md` carries the one-off ClickHouse deletion for rows recorded before this change. Ten locales throughout.
...
- **Owner worklist: "Affiliations & open access" — an editor-only, collapsed panel that helps with the record and judges nothing.** The researcher-first half of reconciling a CV with an institution's record. In the editor's Content region (never on the CV, the public page or any export) the owner sees: (a) current positions whose organisation has no ROR record ("add it on ORCID or pick it in the editor"); (b) works dated during a position they are listed under — the ROR ids they consented to on the institution page, and only those; no lineage folding, which the institution page does and prints — whose affiliation on the paper, as OpenAlex indexes it, names none of those institutions, grouped by the ROR the paper _does_ carry; OpenAlex works with no affiliation data — no institution on the owner's authorship, or none with a ROR id — sit in their own bucket ("missing data, not a missing affiliation"), never in the first, and works from other sources (datasets, conference papers, claimed DOIs), which never carry that data, are counted on one line and not checked; (c) the countable works with no open copy found, each with one of four labels derived only from stored fields (open with a Creative Commons licence / open with another or unknown licence / no open copy found / not determined), a link to the journal's self-archiving policy on Open Policy Finder (by journal name — no ISSN is stored — pointing at the site root with the name in a search parameter, since the deep-link pattern could not be verified), and, as context only, the funder names printed on the work — no mandate, no route, nothing that says a policy applies. No heading carries a count or a share, and there is no open-access breakdown — actions, never states; every row jumps to the entry; the panel is absent when there is nothing to show and in the no-login preview. The vocabulary is enforced: a test fails the build if any of the panel's strings, in any of the ten locales, says compliant, non-compliant, overdue or violation (or their translations). No new export: the owner freezes a version as today.
- **OAI-PMH: an OpenAIRE-Guidelines format (`metadataPrefix=oaire`) beside Dublin Core.** The endpoint only ever spoke `oai_dc`, whose per-work records name creators as strings and carry the owner's ORCID on the CV-level record alone — a CRIS or OpenAIRE could not match a harvested work by identifier. `ListMetadataFormats` now lists `oaire` (OpenAIRE Guidelines for Literature Repositories v4: `oaire:resource` with DataCite kernel-4 elements and COAR vocabularies), and `GetRecord` / `ListRecords` / `ListIdentifiers` honour it for the CV-level and the per-work records; anything else is `cannotDisseminateFormat`. Per work: the owner's ORCID as a `datacite:nameIdentifier` on their OWN author entry (located by the identifier-derived author position, never by name) and on nobody else's — co-author ORCIDs stay out, as they are stripped from the public projection; the DOI; `dateType="Issued"`; a COAR resource type from the CV's own routing then the CSL type (preprint / dataset / software / journal article / conference paper / … / other); a COAR access right derived honestly from the stored open-access determination (`open access` when OpenAlex found an open copy, `metadata only access` when it found none — never "restricted", which the data does not know, and no compliance state anywhere — omitted when undetermined); the reuse licence when the stored slug is a known Creative Commons licence; venue, volume, issue and page range as `oaire:citation*`; and the CV page as a related identifier. A work without a DOI is identified by its own entry's URL on the public page (`identifierType="URL"`), never an invented identifier. No per-work affiliation: the owner's affiliation on a given work is not public, and a current affiliation is not a historical one — the self-declared current affiliation rides the CV-level record's creator instead, as a plain name, and only when the owner also opted into "list under my current affiliation" (the same consent that gates the `ror:<id>` sets). A review report (OpenAlex `peer-review`) and a pre-registration are "other": the v4 vocabulary has no concept for either (COAR `c_ba08` is a _book_ review). `oaire:version` and funders are omitted (unknown / not stored yet). Both record kinds validate against the XSDs OpenAIRE hosts for the format. Resumption tokens carry the format, so page 2 of an `oaire` harvest is still `oaire` (an `oai_dc` token is unchanged). The consent gates are untouched — published + indexable, sets by opt-in — and no field appears in `oaire` that the public page or its `.json` does not already expose; the `oai_dc` output is byte-identical to before (regression-tested). The indexing consent copy (ten locales) now says "in Dublin Core or OpenAIRE format", and `docs/OPEN-SCIENCE.md` describes the format.
- **"Show me on my institution's public page" — a fourth, separate consent, pinned to the institutions you tick.** The existing affiliation listing follows the first current position on every save, so a consent given at one university would silently re-attach to the next. The new consent stores the ROR ids the researcher ticks among their visible current positions (name + ROR id in the picker, dual appointments included), is validated against the stored CV (an id that is not a current affiliation is refused), and is never moved: when a consented affiliation ends, the consent for it lapses — not shown, and the editor asks again ("Your current affiliation changed: confirm whether to be listed under …") rather than re-attaching it. The editor says "You are listed under X"; withdrawal is possible in every state and purges the cached institution pages at once; turning indexing off or unpublishing clears it, exactly like the affiliation listing. The consent copy states what appears (name, ORCID iD, current position and the works the public page lists; you are counted in the page's figures — how many researchers list it, and their works' open-access status — and where OpenAlex's record and yours differ, only counts are shown), that listing is voluntary and absence means nothing, and that withdrawal is immediate — in ten locales; the privacy notice gains the matching sentence and the GDPR export carries both new fields. With several current affiliations, ticking the toggle only opens the picker — nothing is consented until an institution is ticked (with one affiliation, one click consents to it, and the copy names it). A lapsed id is always shown in the editor with its own Remove, is kept when a new affiliation is ticked, and resumes if that affiliation becomes current again; deleting the account purges the cached institution pages too. (The public institution page itself ships separately; this is the consent it reads.)
- **Institution pages: `/i` and `/i/<ror>`, counts only** (the OpenAlex table's open share came later — see above)**.** Every institution at least one researcher chose to be listed under (the existing "list me under my current affiliation for repositories" opt-in) now has a public page in ten languages: its canonical name, its ROR record, "N researchers list this affiliation", a link to the OAI-PMH `ror:<id>` set, and an "About this page" block stating who the controller is, that the institution is a reader that receives nothing and instructs nothing, that listing is voluntary and absence means nothing, and that SigmaCV does not rank, score or compare researchers (with a link to the FAQ on request links). No roster, no per-person column, no ratio, no score — and no OpenAlex or other external call at request time: the page reads the database only. An id that is not ROR-shaped, or a ROR nobody is listed under, is a 404 with a short negative cache; both pages share the public CV pages' rate limit (a rate-limited hit is served as a `noindex, nofollow` notice); the sitemap lists the index and each listed institution with hreflang alternates; a schema.org `Organization` (ROR IRI as `@id`, the OAI set as `subjectOf`, never `employee`/`member`) is embedded. The institution's public name is **ROR's own record**, written into a new `Institution` table during sync for every ROR that resolves on a position — never text from a CV, since an owner can put any string beside any ROR id on a manual position; with no record yet the page, the index, the sitemap, the JSON-LD and the OAI `ListSets` set name all say `ROR <id>`. A page with a single listed researcher is served but `noindex` and left out of the sitemap (it would be that person's affiliation under the institution's name in search results). The listing opt-in's copy now says the count is public on the institution page, and the privacy notice states that per-institution count as a purpose (one sentence each, ten locales) — no other consent copy changes.
...
- **Sixty closed papers read as a list again.** Each closed work in the deposit worklist showed five to ten lines of policy text. It now shows two — the title with its journal and open-access label, then the one deposit action with Copy DOI — and one disclosure per work holds the publisher's record, the statutory rule, the form notes and the other places. Works the record names a version for come first, then those with no record, then "only if your agreement allows", then works with no deposit action. The panel is titled for what it is now that it has its own tab ("What you can act on"), the open-access label "Not determined" reads "Unknown", and the accessibility audit's findings are in: the title is the tab's second-level heading and the groups third-level, "DOI copied" is announced once in a single status region rather than by a live region on every button, every external link and every jump button is described by a hidden note, and the Copy DOI button meets the minimum target size.
...
- **Custom sections are now discoverable.** You could always add your own free-text
  ...
  but it appeared in the add-section menu under its old default title "Statement", so
  people didn't realise they could add "something the preset sections don't cover".
  It's now called **"Custom section"** throughout — the menu button _and_ the section
  it creates (which opens expanded, ready to rename) — with a hint explaining what
  it's for. The underlying capability (add / rename / reorder / hide, multiple
  instances) is unchanged. Prompted by tester feedback.
