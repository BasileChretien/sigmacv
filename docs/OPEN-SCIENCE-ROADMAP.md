...
| 1.7 | CRediT roles `meta.creditRoles` (optional, larger). **Partial:** read from the publisher's Crossref deposit (schema 5.4+; the owner's contributor entry matched by ORCID, never by name) via `enrichCvWithCreditRoles`, plus a self-declaration picker in the editor (`setCreditRoles`, labelled "self-declared" wherever rendered; never overwritten by Crossref). Opt-in render (`display.showCreditRoles`, HTML/PDF). Remaining blocker: Crossref CRediT coverage is small and growing — most works carry none, so the self-declared path fills the gap, explicitly labelled. | `canonical/credit.ts`, `schema.ts`, `build.ts`, `enrich.ts`, `curate.ts`, `crossref/client.ts`, `render/creditRoles.ts` | 🔜 partial |
...
(`canonical/cvModels.ts`): **61 models** across grant funders (~40),
...
| 7.3 | ~~"Narrative CV" starter layout~~ → **CV-model catalog** (`cvModels.ts`, 61 models, grouped picker)                                                                                            | ✅ (live E2E green)              |
