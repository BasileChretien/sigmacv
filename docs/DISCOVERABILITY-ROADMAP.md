...
- **`/guides` surface** (B1) + first 2 cornerstone guides (B2) — typed, English-first content under `src/lib/guides/`, rendered at `/guides` + `/guides/[slug]` with Article + BreadcrumbList + FAQPage + ItemList JSON-LD, a named-author (E-E-A-T) byline, sitemap entries, and a homepage-footer link. Adding more guides = data-only.
...
| B2  | **Cornerstone guides** (definitive, linkable, LLM-citable): "The complete guide to the academic CV", "Academic CV vs résumé", "How to list publications on a CV", "Academic CV for grad-school applications", "How long should an academic CV be", "Academic CV format by country", "Responsible use of metrics on a CV (DORA/Leiden)". _(Shipped 2/7: "How to write an academic CV" + "Academic CV vs résumé". Remaining ones are now trivial data-only additions to `src/lib/guides/guides.ts`.)_ | 🧑‍💻 · 🙋 review | XL     | 🔜     |
