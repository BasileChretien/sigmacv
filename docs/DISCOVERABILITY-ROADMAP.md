...
| C5  | **Examples / templates gallery** — real (illustrative) academic-CV examples by field/career stage; highly linkable + high-intent ("academic CV example/template"). _(Done: `/examples` + 8 illustrative example CVs (biology/CS/psychology/economics/chemistry/physics/history/public-health × grad→faculty), each a FICTIONAL researcher with fabricated publications rendered inline (indexable) + self-name highlight + disclaimer; in `src/lib/examples/`, English-only, footer-linked + sitemapped, cross-linked to persona/landing pages.)_ | 🧑‍💻                    | L      | ✅     |
...
| #   | Item                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Owner                 | Effort | Status |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- | ------ | ------ |
| I1  | **Localized head terms** — target the lower-competition non-English equivalents you can already serve via hreflang: _CV académique / universitaire_ (fr), _wissenschaftlicher Lebenslauf_ (de), _CV académico_ (es), _学术简历_ (zh), _履歴書 / アカデミックCV_ (ja), etc. Localized landing pages for the top markets. _(In progress: the **`/guides` and `/glossary`** surfaces are now localized ×10 — each guide/term is a crawlable `/{loc}/…` URL with reciprocal hreflang, localized JSON-LD (`inLanguage`, breadcrumb labels) and localized internal CTAs. Structure is single-sourced (`*_META`); only prose is per-locale (`*_CONTENT`), with a parity test. Non-EN copy machine-drafted, flagged for native review. The **native head-term landing pages** are also done: 9 single-locale pages at native URLs (`/fr/cv-academique`, `/de/wissenschaftlicher-lebenslauf`, `/es/cv-academico`, `/it/cv-accademico`, `/pt/curriculo-academico`, `/ru/akademicheskoe-rezume`, `/zh/xueshu-jianli`, `/ja/gakujutsu-cv`, `/ko/haksul-cv`) with native-first copy, kept out of the ×10 machinery (own routes + sitemap), rendered through the shared `LandingPage`.)_ | 🧑‍💻 · 🙋 native review | L      | ✅     |
