...
**Which file, in plain words.** Under the action, a visible line says what to upload (`depositActionVersion` → `wlFile*`): for the accepted manuscript, "your own manuscript as accepted after peer review, without the journal's copy-editing and layout. Not the publisher's PDF."; for the submitted manuscript, the one first submitted; for the published version, "the publisher's PDF". The version is the one the action names, branch for branch — a HAL notice takes the ground's; an action that names none (only if the agreement allows it: the record does not cover the place) names no file either. The reason sentence under it names the ground only — the publisher's record and its date, the law and its delay, the work's licence — so each row names the version once in the action and once in plain words. Known limit: under the Dutch and Bulgarian rules the line still says the accepted manuscript, which is stricter than those texts.
