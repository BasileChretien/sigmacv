...
| `10.13039/100010269` | Wellcome                            | Europe PMC plus        | https://plus.europepmc.org/ | **pending** | Europe PMC plus, read in a browser on 2026-09-15, takes accepted manuscripts of research funded by a Europe PMC funder. Wellcome's own policy page could not be reached (404 at its former addresses). A pending entry is **never routed to**: confirm the policy, then set `verifiedBy: "maintainer"` and `lastVerified`. |
