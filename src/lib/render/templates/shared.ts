...
export function commonCss(theme: TemplateTheme): string {
  ...
  * { box-sizing: border-box; }
  ...
  /* PDF/print page box. The horizontal gutter is left to .cv's own
     ...
     ZERO page margin: every margin a template wants comes from its OWN box (the
     centred templates' .cv padding; the Sidebar's panel + main-column padding), so
     the PDF and the on-screen preview render the SAME box with no page gutter to
