...
export const atsTemplate: CvTemplate = {
  ...
  render(cv, sections, theme, opts) {
    ...
    const body = `<div class="cv">${headerHtml(cv)}${sectionsHtml(cv, sections)}${provenanceFooter(cv, opts)}${licenseFooter(cv)}${coauthorLinksFooter(cv, opts)}${attributionFooter(cv, opts)}</div>`;
