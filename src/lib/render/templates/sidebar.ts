...
export const sidebarTemplate: CvTemplate = {
  ...
  render(cv, sections, theme, opts) {
    ...
    const body =
      ...
      `<main class="cv-main">${sectionsHtmlRaw(cv, sections, opts)}${provenanceFooter(cv, opts)}${licenseFooter(cv)}${coauthorLinksFooter(cv, opts)}${attributionFooter(cv, opts)}${docQrFooter(cv, opts)}</main>` +
      `</div>` +
      `</div>`;
