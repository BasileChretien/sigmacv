...
export const classicTemplate: CvTemplate = {
  ...
  render(cv, sections, theme, opts) {
    ...
    const body = `<div class="cv">${headerHtml(cv)}${sectionsHtml(cv, sections, opts)}${provenanceFooter(cv, opts)}${licenseFooter(cv)}${coauthorLinksFooter(cv, opts)}${attributionFooter(cv, opts)}${docQrFooter(cv, opts)}</div>`;
