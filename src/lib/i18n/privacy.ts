...
const PRIVACY_I18N: Record<Locale, PrivacyStrings> = {
  ...
  "zh-CN": {
    ...
    data: "来自您登录提供方的账户数据（ORCID iD，或电子邮件 / Google 账户）；用于生成您简历的公开研究元数据（来自 OpenAlex、ORCID、Crossref、DataCite 和 Open Editors Plus 的出版物、资助、所属机构和指标）；您创建的简历内容和显示选择；以及您上传的可选个人照片。除非您选择发布公开页面，否则您的简历是私密的。如果您冻结了简历的某个版本，该冻结副本（不含您的私人笔记）会与您的账户一起保存，仅在您设为公开时才会公开，并会随您的其他数据一起导出和删除。",
  ...
  "pt-BR": {
    ...
    data: "Dados de conta do seu provedor de login (ORCID iD, ou conta de e-mail / Google); os metadados públicos de pesquisa usados para montar seu CV (publicações, financiamentos, afiliações e métricas de OpenAlex, ORCID, Crossref, DataCite e Open Editors Plus); o conteúdo do CV e as escolhas de exibição que você cria; e uma foto de perfil opcional que você carrega. Seu CV é privado, a menos que você opte por publicar uma página pública. Se você congelar uma versão do seu CV, essa cópia congelada (sem suas notas privadas) é mantida com sua conta, só é pública se você assim decidir, e é exportada e excluída junto com o restante dos seus dados.",
