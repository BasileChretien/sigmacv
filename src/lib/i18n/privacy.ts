...
const PRIVACY_I18N: Record<Locale, PrivacyStrings> = {
  ...
  "zh-CN": {
    ...
    sharing:
      "我们绝不出售您的数据，也不将其用于广告。我们仅为运营本服务而共享数据：与位于欧盟（德国）的托管服务商共享，其依据数据处理协议代表我们存储数据；以及在您登录或同步时，与我们按您的标识符查询以生成您简历的公共研究服务共享（OpenAlex、ORCID、Crossref、DataCite、OpenAIRE、DBLP、ROR 等），这些服务接收您的 ORCID iD 或姓名以返回您的公开记录，并作为该公开数据的独立控制者。您的账户和简历存储在欧盟；数据控制者位于日本，日本与欧盟之间的传输受 2019 年日本—欧盟相互充分性认定的保护。当您选择接收邮件摘要或确认联系邮箱时，我们会通过 SMTP2GO 发送这些邮件——SMTP2GO 是一家位于新西兰的邮件递送处理方，仅接收您的邮箱地址和邮件内容（对于摘要，即您本人公开成果的标题），以代表我们发送邮件；欧盟已认定新西兰能够确保充分的数据保护水平。",
