...
const SECTION_TITLES: Record<Locale, Record<CvSectionType, string>> = {
  ...
  "es-ES": {
    ...
    statement: "Sección personalizada",
    teaching: "Docencia",
  ...
  "fr-FR": {
    ...
    statement: "Section personnalisée",
  ...
  "it-IT": {
    ...
    statement: "Sezione personalizzata",
    teaching: "Didattica",
...
const EN = {
  ...
  misWhyPrefix: "OpenAlex matched this by author profile, and it differs from your other work:",
  ...
  publicationNameLabel: "Name on your publications",
  publicationNameHint:
    "Override how your own name appears in your publication list — e.g. a surname change, or a form your data source normalised. Leave blank to use the source name. Applies only to works matched to you by your identifier.",
  publicationNameGiven: "Given name(s)",
  publicationNameFamily: "Family name",
  addCustomSectionHint:
    "A section with your own heading and free text — for anything the preset sections don't cover.",
...
const ZH: Record<ChromeKey, string> = {
  ...
  misWhyPrefix: "OpenAlex 依据作者档案匹配到此项，但它与您的其他成果不同：",
  ...
  sourceManual: "由您添加",
  addSection: "添加部分",
  publicationNameLabel: "出版物上的姓名",
  publicationNameHint:
    "自定义你本人姓名在出版物列表中的显示方式——例如姓氏变更，或数据源规范化后的形式。留空则使用数据源中的姓名。仅适用于通过标识符与你匹配的成果。",
  publicationNameGiven: "名",
  publicationNameFamily: "姓",
  addCustomSectionHint: "带有自定义标题和自由文本的部分——用于预设部分未涵盖的任何内容。",
  expandSection: "展开该部分",
...
const ES: Record<ChromeKey, string> = {
  ...
  misWhyPrefix: "OpenAlex lo emparejó por perfil de autor y difiere de tus otros trabajos:",
  ...
  publicationNameLabel: "Nombre en tus publicaciones",
  publicationNameHint:
    "Personaliza cómo aparece tu propio nombre en tu lista de publicaciones (p. ej., un cambio de apellido o una forma que tu fuente normalizó). Déjalo en blanco para usar el nombre de origen. Solo se aplica a los trabajos vinculados a ti por tu identificador.",
  publicationNameGiven: "Nombre(s) de pila",
  publicationNameFamily: "Apellido(s)",
  addCustomSectionHint:
    "Una sección con tu propio título y texto libre, para lo que las secciones predefinidas no cubran.",
...
const FR: Record<ChromeKey, string> = {
  ...
  misWhyPrefix: "OpenAlex l’a apparié par profil d’auteur, et il diffère de vos autres travaux :",
  ...
  publicationNameLabel: "Nom sur vos publications",
  publicationNameHint:
    "Personnalisez l'affichage de votre propre nom dans votre liste de publications (par ex. un changement de nom, ou une forme normalisée par votre source). Laissez vide pour utiliser le nom source. S'applique uniquement aux travaux rattachés à vous par votre identifiant.",
  publicationNameGiven: "Prénom(s)",
  publicationNameFamily: "Nom de famille",
  addCustomSectionHint:
    "Une section avec votre propre titre et du texte libre, pour tout ce que les sections prédéfinies ne couvrent pas.",
...
const DE: Record<ChromeKey, string> = {
  ...
  misWhyPrefix:
    "OpenAlex hat dies über ein Autorenprofil zugeordnet, und es weicht von Ihren anderen Arbeiten ab:",
  ...
  publicationNameLabel: "Name in Ihren Publikationen",
  publicationNameHint:
    "Legen Sie fest, wie Ihr eigener Name in Ihrer Publikationsliste erscheint – z. B. bei einer Namensänderung oder einer von Ihrer Quelle normalisierten Form. Leer lassen, um den Quellnamen zu verwenden. Gilt nur für Ihnen per Kennung zugeordnete Arbeiten.",
  publicationNameGiven: "Vorname(n)",
  publicationNameFamily: "Nachname",
  addCustomSectionHint:
    "Ein Abschnitt mit eigener Überschrift und Freitext – für alles, was die vorgegebenen Abschnitte nicht abdecken.",
...
const JA: Record<ChromeKey, string> = {
  ...
  misWhyPrefix: "OpenAlex が著者プロフィールで照合しましたが、他のご業績とは異なります：",
  ...
  sourceManual: "あなたが追加",
  addSection: "セクションを追加",
  publicationNameLabel: "出版物での氏名",
  publicationNameHint:
    "出版物リストでの自分の氏名の表示方法を上書きします（姓の変更や、データソースが正規化した形など）。空欄の場合はソースの氏名を使用します。識別子であなたに紐づいた成果にのみ適用されます。",
  publicationNameGiven: "名",
  publicationNameFamily: "姓",
  addCustomSectionHint:
    "見出しと自由記述を自分で設定できるセクション。既定のセクションでは足りない内容に。",
...
const PT: Record<ChromeKey, string> = {
  ...
  misWhyPrefix:
    "O OpenAlex correspondeu isto por perfil de autor, e difere dos seus outros trabalhos:",
  ...
  publicationNameLabel: "Nome nas suas publicações",
  publicationNameHint:
    "Personalize como o seu próprio nome aparece na sua lista de publicações — por exemplo, uma mudança de sobrenome ou uma forma que a sua fonte normalizou. Deixe em branco para usar o nome de origem. Aplica-se apenas aos trabalhos vinculados a você pelo seu identificador.",
  publicationNameGiven: "Nome(s)",
  publicationNameFamily: "Sobrenome",
  addCustomSectionHint:
    "Uma seção com seu próprio título e texto livre — para o que as seções predefinidas não cobrem.",
...
const IT: Record<ChromeKey, string> = {
  ...
  misWhyPrefix:
    "OpenAlex lo ha abbinato tramite profilo autore e differisce dai tuoi altri lavori:",
  ...
  publicationNameLabel: "Nome nelle tue pubblicazioni",
  publicationNameHint:
    "Personalizza come appare il tuo nome nell'elenco delle pubblicazioni — ad es. un cambio di cognome o una forma normalizzata dalla fonte. Lascia vuoto per usare il nome di origine. Si applica solo ai lavori collegati a te tramite il tuo identificativo.",
  publicationNameGiven: "Nome",
  publicationNameFamily: "Cognome",
  addCustomSectionHint:
    "Una sezione con un titolo tuo e testo libero, per tutto ciò che le sezioni predefinite non coprono.",
...
const KO: Record<ChromeKey, string> = {
  ...
  misWhyPrefix: "OpenAlex가 저자 프로필로 매칭했으나, 다른 업적과 다릅니다:",
  ...
  sourceManual: "직접 추가함",
  addSection: "섹션 추가",
  publicationNameLabel: "출판물에 표시되는 이름",
  publicationNameHint:
    "출판물 목록에 표시되는 본인 이름을 원하는 대로 바꿉니다 — 예: 성 변경, 또는 데이터 출처가 정규화한 형태. 비워 두면 출처의 이름을 사용합니다. 식별자로 본인과 매칭된 성과에만 적용됩니다.",
  publicationNameGiven: "이름",
  publicationNameFamily: "성",
  addCustomSectionHint: "제목과 자유 텍스트를 직접 정하는 섹션 — 기본 섹션에 없는 내용을 위해.",
...
const RU: Record<ChromeKey, string> = {
  ...
  misWhyPrefix: "OpenAlex сопоставил это по профилю автора, и работа отличается от ваших других:",
  ...
  publicationNameLabel: "Имя в ваших публикациях",
  publicationNameHint:
    "Измените, как ваше собственное имя отображается в списке публикаций — например, при смене фамилии или из-за формы, нормализованной источником. Оставьте пустым, чтобы использовать имя из источника. Применяется только к работам, сопоставленным с вами по идентификатору.",
  publicationNameGiven: "Имя",
  publicationNameFamily: "Фамилия",
  addCustomSectionHint:
    "Раздел с собственным заголовком и произвольным текстом — для всего, что не охватывают готовые разделы.",
