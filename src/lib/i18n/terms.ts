...
const TERMS_I18N: Record<Locale, TermsStrings> = {
  "en-US": {
    ...
    lookup:
      "The public ORCID preview lets you view a record assembled from public sources about a researcher who may never have used SigmaCV. Such a preview is automatic and unreviewed by the person it describes, may attribute work wrongly, and is not an assessment, ranking or endorsement of anyone by SigmaCV. Use it only to find or check public research records. Do not use it to make automated decisions about a person, to profile or harass anyone, or to extract records in bulk, and respect the wishes of a researcher who has asked us not to show their record. The organisation view (/i/compare) sets public OpenAlex counts side by side; it is a description, not an assessment, ranking or endorsement of any organisation. Do not republish it as a ranking or league table, use it for automated decisions about an organisation or its staff, or extract it in bulk. The comparison's JSON export is public data under CC0, which places no condition on its reuse; SigmaCV asks, without requiring it, that reusers cite OpenAlex and SigmaCV and do not republish it as a ranking or league table.",
  ...
  "zh-CN": {
    ...
    lookup:
      "公开 ORCID 预览允许您查看依据公开数据源生成的、关于某位可能从未使用过 SigmaCV 的研究者的记录。此类预览是自动生成的，未经其所描述的本人审核，可能存在错误归属，并且不构成 SigmaCV 对任何人的评估、排名或背书。请仅将其用于查找或核对公开的研究记录。请勿将其用于对个人作出自动化决定、对任何人进行画像或骚扰，或批量提取记录；并请尊重已要求我们不显示其记录的研究者的意愿。机构并列查看（/i/compare）将 OpenAlex 的公开数量并列展示；它是描述，而不是对任何机构的评估、排名或背书。请勿将其重新发布为排名或排行榜，请勿用于对机构或其人员作出自动化决定，也请勿批量提取。并列数据的 JSON 导出为 CC0 许可的公开数据，CC0 对其再利用不附加任何条件；SigmaCV 仅请求（而非要求）再利用者引用 OpenAlex 与 SigmaCV，并且不要将其重新发布为排名或排行榜。",
  ...
  "de-DE": {
    ...
    lookup:
      "Die öffentliche ORCID-Vorschau erlaubt es Ihnen, einen aus öffentlichen Quellen zusammengestellten Nachweis über eine forschende Person zu sehen, die SigmaCV möglicherweise nie genutzt hat. Eine solche Vorschau ist automatisch erstellt, von der beschriebenen Person nicht geprüft, kann Werke falsch zuordnen und ist keine Bewertung, kein Ranking und keine Empfehlung irgendeiner Person durch SigmaCV. Nutzen Sie sie nur, um öffentliche Forschungsnachweise zu finden oder zu prüfen. Nutzen Sie sie nicht für automatisierte Entscheidungen über eine Person, zum Profiling oder zur Belästigung und nicht zur massenhaften Extraktion von Nachweisen; respektieren Sie den Wunsch einer forschenden Person, die uns gebeten hat, ihren Nachweis nicht anzuzeigen. Die Einrichtungsansicht (/i/compare) stellt öffentliche OpenAlex-Zahlen nebeneinander; sie ist eine Beschreibung, keine Bewertung, Rangliste oder Empfehlung einer Einrichtung. Veröffentlichen Sie sie nicht als Rangliste oder Tabelle weiter, nutzen Sie sie nicht für automatisierte Entscheidungen über eine Einrichtung oder deren Beschäftigte, und extrahieren Sie sie nicht massenhaft. Der JSON-Export der Gegenüberstellung besteht aus öffentlichen Daten unter CC0, das an die Weiterverwendung keine Bedingung knüpft; SigmaCV bittet, ohne es zu verlangen, darum, OpenAlex und SigmaCV zu zitieren und den Export nicht als Rangliste oder Tabelle weiterzuveröffentlichen.",
  ...
  "ja-JP": {
    ...
    lookup:
      "ORCID による公開プレビューでは、SigmaCV を一度も利用したことがないかもしれない研究者について、公開ソースから組み立てた記録を見ることができます。このプレビューは自動生成であり、記載された本人による確認を経ておらず、業績が誤って帰属されている場合があり、SigmaCV による評価・順位付け・推薦ではありません。公開研究記録を探す、または確認する目的にのみご利用ください。個人に関する自動化された決定、プロファイリングや嫌がらせ、記録の大量抽出に利用しないでください。また、自身の記録を表示しないよう当方に求めた研究者の意思を尊重してください。機関の並置（/i/compare）は OpenAlex の公開件数を並べて示すものであり、いかなる機関に対する評価・順位付け・推薦でもなく、記述にすぎません。順位表やランキングとして再公開しないでください。機関やその職員に関する自動化された決定に利用したり、大量に抽出したりしないでください。並置データの JSON 出力は CC0 の公開データであり、CC0 はその再利用に何の条件も課しません。SigmaCV は、義務ではなくお願いとして、再利用の際に OpenAlex と SigmaCV を引用し、順位表やランキングとして再公開しないよう求めます。",
    warrantyHeading: "無保証",
    warranty:
  ...
  "pt-BR": {
    ...
    lookup:
      "A prévia pública por ORCID permite ver um registro montado a partir de fontes públicas sobre um pesquisador que pode nunca ter usado o SigmaCV. Essa prévia é automática, não foi revisada pela pessoa descrita, pode atribuir trabalhos erroneamente e não é uma avaliação, classificação ou endosso de ninguém pelo SigmaCV. Use-a apenas para localizar ou verificar registros públicos de pesquisa. Não a use para tomar decisões automatizadas sobre uma pessoa, para traçar perfis ou assediar alguém, nem para extrair registros em massa, e respeite a vontade de um pesquisador que nos pediu para não exibir o seu registro. A visão de organizações (/i/compare) coloca contagens públicas do OpenAlex lado a lado; é uma descrição, não uma avaliação, classificação ou endosso de qualquer organização. Não a republique como classificação ou tabela de posições, não a use para decisões automatizadas sobre uma organização ou seu pessoal, nem a extraia em massa. A exportação JSON da comparação consiste em dados públicos sob CC0, que não impõe condição alguma à sua reutilização; o SigmaCV pede, sem exigir, que quem a reutilizar cite o OpenAlex e o SigmaCV e não a republique como classificação ou tabela de posições.",
  ...
  "it-IT": {
    ...
    lookup:
      "L'anteprima pubblica tramite ORCID ti permette di vedere una registrazione assemblata da fonti pubbliche su un ricercatore che potrebbe non aver mai usato SigmaCV. Tale anteprima è automatica, non è stata verificata dalla persona descritta, può attribuire lavori in modo errato e non costituisce una valutazione, una classifica o un avallo di chicchessia da parte di SigmaCV. Usala solo per trovare o verificare registrazioni pubbliche della ricerca. Non usarla per prendere decisioni automatizzate su una persona, per profilare o molestare qualcuno, né per estrarre registrazioni in massa, e rispetta la volontà di un ricercatore che ci ha chiesto di non mostrare la sua registrazione. La vista delle organizzazioni (/i/compare) affianca conteggi pubblici di OpenAlex; è una descrizione, non una valutazione, una classifica o un avallo di alcuna organizzazione. Non ripubblicarla come classifica o graduatoria, non usarla per decisioni automatizzate su un'organizzazione o il suo personale, né estrarla in massa. L'esportazione JSON del confronto è un dato pubblico sotto CC0, che non pone alcuna condizione al suo riutilizzo; SigmaCV chiede, senza esigerlo, che chi la riutilizza citi OpenAlex e SigmaCV e non la ripubblichi come classifica o graduatoria.",
  ...
  "ko-KR": {
    ...
    lookup:
      "ORCID 공개 미리보기를 통해 SigmaCV를 한 번도 사용하지 않았을 수 있는 연구자에 대해 공개 소스로 구성된 기록을 볼 수 있습니다. 이러한 미리보기는 자동으로 생성되며, 기술된 본인의 검토를 거치지 않았고, 성과가 잘못 귀속될 수 있으며, SigmaCV가 누군가를 평가·순위화·보증하는 것이 아닙니다. 공개 연구 기록을 찾거나 확인하는 용도로만 사용하십시오. 개인에 관한 자동화된 결정, 프로파일링이나 괴롭힘, 기록의 대량 추출에 사용하지 마시고, 자신의 기록을 표시하지 말아 달라고 요청한 연구자의 뜻을 존중해 주십시오. 기관 나란히 보기(/i/compare)는 OpenAlex의 공개 건수를 나란히 놓은 것으로, 어떤 기관에 대한 평가·순위화·보증이 아니라 기술입니다. 순위표나 랭킹으로 재게시하거나, 기관이나 그 직원에 관한 자동화된 결정에 사용하거나, 대량으로 추출하지 마십시오. 비교 데이터의 JSON 내보내기는 CC0 공개 데이터이며, CC0는 그 재사용에 아무 조건도 두지 않습니다. SigmaCV는 의무가 아닌 부탁으로서, 재사용 시 OpenAlex와 SigmaCV를 인용하고 순위표나 랭킹으로 재게시하지 말아 달라고 요청합니다.",
  ...
  "ru-RU": {
    ...
    lookup:
      "Публичный предпросмотр по ORCID позволяет увидеть запись, собранную из открытых источников, об исследователе, который, возможно, никогда не пользовался SigmaCV. Такой предпросмотр формируется автоматически, не проверен описанным в нём человеком, может содержать ошибочную атрибуцию работ и не является оценкой, рейтингом или рекомендацией кого-либо со стороны SigmaCV. Используйте его только для поиска или проверки открытых научных записей. Не используйте его для автоматизированных решений о человеке, для профилирования или преследования кого-либо, а также для массового извлечения записей, и уважайте волю исследователя, попросившего нас не показывать его запись. Представление организаций (/i/compare) ставит рядом публичные числа OpenAlex; это описание, а не оценка, рейтинг или одобрение какой-либо организации. Не публикуйте его повторно как рейтинг или таблицу, не используйте для автоматизированных решений об организации или её сотрудниках и не извлекайте массово. JSON-экспорт сопоставления — публичные данные под CC0, которая не ставит никаких условий для его повторного использования; SigmaCV просит, не требуя этого, при повторном использовании цитировать OpenAlex и SigmaCV и не публиковать его как рейтинг или таблицу.",
