...
const TERMS_I18N: Record<Locale, TermsStrings> = {
  ...
  "zh-CN": {
    ...
    lookup:
      "公开 ORCID 预览允许您查看依据公开数据源生成的、关于某位可能从未使用过 SigmaCV 的研究者的记录。此类预览是自动生成的，未经其所描述的本人审核，可能存在错误归属，并且不构成 SigmaCV 对任何人的评估、排名或背书。请仅将其用于查找或核对公开的研究记录。请勿将其用于对个人作出自动化决定、对任何人进行画像或骚扰，或批量提取记录；并请尊重已要求我们不显示其记录的研究者的意愿。机构并列查看（/i/compare）将 OpenAlex 的公开数量并列展示；它是描述，而不是对任何机构的评估、排名或背书。请勿将其重新发布为排名或排行榜，请勿用于对机构或其人员作出自动化决定，也请勿批量提取。",
