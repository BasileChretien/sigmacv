...
const PAGE = "latex-cv" as const;

/** Localized "LaTeX CV" landing page — a crawlable URL per language with
...
export default async function LocaleLatexCvPage({ params }: Params) {
  const { locale: slug } = await params;
  const loc = localeForSlug(slug);
