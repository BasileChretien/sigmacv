...
const PAGE = "funder-cv-templates" as const;

/** Localized "Funder CV templates" landing page — a crawlable URL per language
 *  with reciprocal hreflang. The default (en-US) lives at the bare path. */
...
export default async function LocaleFunderCvTemplatesPage({ params }: Params) {
  const { locale: slug } = await params;
  const loc = localeForSlug(slug);
