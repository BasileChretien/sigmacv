...
describe("splitSelf", () => {
  it("returns a single non-self segment when there are no usable variants", () => {
    expect(splitSelf("Chrétien, B.", [])).toEqual([
      { text: "Chrétien, B.", self: false },
    ]);
  });

  it("ignores variants shorter than 2 chars", () => {
    expect(splitSelf("A. Smith", ["A"])).toEqual([
      { text: "A. Smith", self: false },
    ]);
  });
...
describe("wrapSelf", () => {
  it("wraps only the self segments", () => {
    expect(wrapSelf("Chrétien, B.", ["Chrétien"], (s) => `**${s}**`)).toBe(
      "**Chrétien**, B.",
    );
  });
