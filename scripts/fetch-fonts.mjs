...
// italic → whether the family ships an italic face (Space Grotesk does not).
const FONTS = [
  { key: "sourceSerif4", family: "Source Serif 4", pkg: "source-serif-4", italic: true },
  { key: "inter", family: "Inter", pkg: "inter", italic: true },
  { key: "ebGaramond", family: "EB Garamond", pkg: "eb-garamond", italic: true },
  // Characterful display faces for the field public-page styles.
  { key: "spaceGrotesk", family: "Space Grotesk", pkg: "space-grotesk", italic: false },
  { key: "newsreader", family: "Newsreader", pkg: "newsreader", italic: true },
  { key: "fraunces", family: "Fraunces", pkg: "fraunces", italic: true },
];
...
async function main() {
  ...
  for (const { key, family, pkg, italic } of FONTS) {
    let css = face(family, "normal", await dataUri(pkg, "normal"));
    if (italic) css += face(family, "italic", await dataUri(pkg, "italic"));
    const banner =
